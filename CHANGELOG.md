...
- Pretraining source registry gate [Data Infra, Pre-training]
  - Goal: Define the minimum source-registry contract for pretraining datasets so manifests can reference approved corpus identities without relying on free-form source naming alone.
  - Kind: `gate`
  - Boundary: `core-contract`
  - Contracts: `artifact`
  - Scope: `docs/adr/`, `ARCHITECTURE.md`, `README.md`, `tests/`

- Deployment verification receipt gate [Serving/Deployment Infra, Experimentation/Eval Infra]
  - Goal: Define the minimum verification receipt contract for promoted artifacts so post-deploy checks can be audited without reconstructing the rollout outcome from raw logs alone.
  - Kind: `gate`
  - Boundary: `adapter-deployment`
  - Contracts: `artifact`, `event`
  - Scope: `docs/adr/`, `ARCHITECTURE.md`, `README.md`, `tests/`

- Artifact cleanup inventory contract [Serving/Deployment Infra, Runtime Infra]
  - Goal: Materialize a structured cleanup-candidate inventory so retention workflows can distinguish retained versus prunable artifacts before any future dry-run receipt summarizes deletion intent.
  - Kind: `mixed`
