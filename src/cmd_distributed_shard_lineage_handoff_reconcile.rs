        ...
        match arg.as_str() {
            ...
            _ => {
                return Err(DistributedShardLineageHandoffReconcileError::InvalidArg(
                    format!(
                        "unknown argument for lineage reconcile-handoff: {arg}\n{}",
                        usage()
                    ),
                ));
            }
...
fn load_json_object(
    path: &Path,
    kind: &str,
) -> Result<serde_json::Map<String, Value>, DistributedShardLineageHandoffReconcileError> {
    let text = fs::read_to_string(path)?;
    let value: Value = serde_json::from_str(&text).map_err(|err| {
        DistributedShardLineageHandoffReconcileError::Parse(format!(
            "parse {kind} `{}`: {err}",
            path.display()
        ))
    })?;
...
fn read_string(
    ...
    path: &Path,
    kind: &str,
) -> Result<String, DistributedShardLineageHandoffReconcileError> {
    object
        .get(key)
...
fn read_u64(
    ...
    path: &Path,
    kind: &str,
) -> Result<u64, DistributedShardLineageHandoffReconcileError> {
...
fn usage() -> &'static str {
    "usage: afterburner lineage reconcile-handoff --bundle PATH --handoff PATH [--out PATH]"
}
