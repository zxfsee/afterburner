...
- Deployment stack launch transport locator adapter [Serving/Deployment Infra, Runtime Infra]
  - Goal: Export one stable transport-facing locator over the deployment stack launch evidence handoff so downstream deployment consumers can resolve the current stack-launch handoff without depending on local bundle or handoff path conventions.
  - Kind: `mixed`
