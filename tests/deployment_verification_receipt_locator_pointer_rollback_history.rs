...
#[test]
fn deployment_verification_receipt_locator_pointer_rollback_history_schema_and_workflow_are_explicit()
 {
    let schema_text = fs::read_to_string(fixture_path(
        "deployment_verification_receipt_locator_pointer_rollback_history.schema.json",
    ))
    .expect("read deployment verification receipt locator rollback history schema");
    let schema: Value = serde_json::from_str(&schema_text)
        .expect("parse deployment verification receipt locator rollback history schema");
    ...
    assert!(
        ...
        "justfile must expose the deployment-verification-record-receipt-locator-rollback-history workflow"
    );

    let workflows = repo_file("docs/workflows.md");
    assert!(
        workflows.contains("deployment_verification_receipt_locator_pointer_rollback_history.json"),
        "workflow reference must mention the deployment verification receipt locator rollback history artifact"
    );
    assert!(
        workflows.contains("just deployment-verification-record-receipt-locator-rollback-history"),
        "workflow reference must mention the deployment verification receipt locator rollback history workflow"
    );
}

#[test]
fn deployment_verification_receipt_locator_pointer_rollback_history_writes_history_and_event() {
    let tmp = tempfile::tempdir().expect("tempdir");
    let rollback = tmp
        .path()
        .join("deployment_verification_receipt_locator_pointer_rollback.json");
    fs::write(
        &rollback,
        r#"{
  "schema_version": "1",
  ...
  "rolled_back_at_unix_ms": 1735689800000
    ...
    let event = stderr
        ...
        .find(|event| {
            event.get("event").and_then(Value::as_str)
                == Some("deployment_verification_receipt_locator_pointer_rollback_history_written")
        })
        .unwrap_or_else(|| {
            panic!(
                "missing deployment verification receipt locator rollback history event in stderr: {stderr}"
            )
        });
    ...
    assert_eq!(
        ...
        serde_json::json!({
            ...
            "level": "info",
            "source": "deploy_cli",
            "event": "deployment_verification_receipt_locator_pointer_rollback_history_written",
            "fields": {
