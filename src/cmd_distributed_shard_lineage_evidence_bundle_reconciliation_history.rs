...
const DEFAULT_OUT_PATH: &str =
    "artifacts/train/distributed_shard_lineage_evidence_bundle_reconciliation_history.json";

#[derive(Debug)]
enum DistributedShardLineageEvidenceBundleReconciliationHistoryError {
    InvalidArg(String),
    Io(std::io::Error),
    Parse(String),
}

impl std::fmt::Display for DistributedShardLineageEvidenceBundleReconciliationHistoryError {
...
impl std::error::Error for DistributedShardLineageEvidenceBundleReconciliationHistoryError {}

impl From<std::io::Error> for DistributedShardLineageEvidenceBundleReconciliationHistoryError {
    fn from(value: std::io::Error) -> Self {
        Self::Io(value)
    }
}

impl From<JsonArtifactError> for DistributedShardLineageEvidenceBundleReconciliationHistoryError {
    fn from(value: JsonArtifactError) -> Self {
        match value {
            JsonArtifactError::Io(err) => Self::Io(err),
            JsonArtifactError::Serialize(err) => Self::Parse(format!(
                "serialize distributed shard lineage evidence bundle reconciliation history json: {err}"
            )),
        }
    }
}
...
fn run_inner<I>(
    args: I,
) -> Result<(), DistributedShardLineageEvidenceBundleReconciliationHistoryError>
where
    I: Iterator<Item = String>,
{
    ...
    let entry = json!({
        ...
        "receipt_path": read_string(&reconciliation, "receipt_path", args.reconciliation.as_path())?,
    ...
    emit_json_artifact_written(
        "train_cli",
        "distributed_shard_lineage_evidence_bundle_reconciliation_history_written",
        "history_path",
        &args.out_path,
        "history",
        history,
...
fn parse_args<I>(
    args: I,
) -> Result<Args, DistributedShardLineageEvidenceBundleReconciliationHistoryError>
where
    I: Iterator<Item = String>,
{
        ...
        match arg.as_str() {
            ...
            _ if arg.starts_with("--recorded-at-unix-ms=") => recorded_at_unix_ms = Some(
                arg.trim_start_matches("--recorded-at-unix-ms=")
                    .parse::<u64>()
                    .map_err(|_| {
                        DistributedShardLineageEvidenceBundleReconciliationHistoryError::InvalidArg(
            ...
            _ => {
                return Err(
                    DistributedShardLineageEvidenceBundleReconciliationHistoryError::InvalidArg(
                        format!(
                            "unknown argument for lineage record-evidence-bundle-reconciliation-history: {arg}\n{}",
                            usage()
                        ),
                    ),
                );
    ...
    Ok(Args {
        reconciliation: reconciliation.ok_or_else(|| {
            DistributedShardLineageEvidenceBundleReconciliationHistoryError::InvalidArg(format!(
...
fn parse_value<T, I>(
    args: &mut I,
    flag: &str,
) -> Result<T, DistributedShardLineageEvidenceBundleReconciliationHistoryError>
where
    T: std::str::FromStr,
    I: Iterator<Item = String>,
{
...
> {
    ...
    let value: Value = serde_json::from_str(&text).map_err(|err| {
        DistributedShardLineageEvidenceBundleReconciliationHistoryError::Parse(format!(
            "parse {kind} `{}`: {err}",
            path.display()
        ))
    })?;
...
fn load_history(
    path: &Path,
) -> Result<Value, DistributedShardLineageEvidenceBundleReconciliationHistoryError> {
    let text = fs::read_to_string(path)?;
    let history: Value = serde_json::from_str(&text).map_err(|err| {
        DistributedShardLineageEvidenceBundleReconciliationHistoryError::Parse(format!(
            "parse distributed shard lineage evidence bundle reconciliation history `{}`: {err}",
            path.display()
        ))
    })?;
    history
        ...
        .ok_or_else(|| {
            DistributedShardLineageEvidenceBundleReconciliationHistoryError::Parse(format!(
                "distributed shard lineage evidence bundle reconciliation history `{}` must contain an `entries` array",
                path.display()
            ))
        })?;
    Ok(history)
...
fn read_string(
    ...
    key: &'static str,
    path: &Path,
) -> Result<String, DistributedShardLineageEvidenceBundleReconciliationHistoryError> {
    object
        .get(key)
        ...
        .ok_or_else(|| {
            DistributedShardLineageEvidenceBundleReconciliationHistoryError::Parse(format!(
                "distributed shard lineage evidence bundle reconciliation `{}` missing string field `{key}`",
                path.display()
            ))
        })
}
...
fn read_object(
    ...
    object.get(key).cloned().ok_or_else(|| {
        DistributedShardLineageEvidenceBundleReconciliationHistoryError::Parse(format!(
            "distributed shard lineage evidence bundle reconciliation `{}` missing field `{key}`",
            path.display()
        ))
    })
}
