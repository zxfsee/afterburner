...
const DEFAULT_OUT_PATH: &str =
    "artifacts/deploy/deployment_stack_launch_evidence_handoff_history.json";

#[derive(Debug)]
enum DeploymentStackLaunchHandoffHistoryError {
    InvalidArg(String),
    Io(std::io::Error),
    Parse(String),
}

impl std::fmt::Display for DeploymentStackLaunchHandoffHistoryError {
...
impl std::error::Error for DeploymentStackLaunchHandoffHistoryError {}

impl From<std::io::Error> for DeploymentStackLaunchHandoffHistoryError {
    fn from(value: std::io::Error) -> Self {
        Self::Io(value)
    }
}

impl From<JsonArtifactError> for DeploymentStackLaunchHandoffHistoryError {
    fn from(value: JsonArtifactError) -> Self {
        match value {
            JsonArtifactError::Io(err) => Self::Io(err),
            JsonArtifactError::Serialize(err) => Self::Parse(format!(
                "serialize deployment stack launch handoff history json: {err}"
            )),
        }
    }
}
...
fn run_inner<I>(args: I) -> Result<(), DeploymentStackLaunchHandoffHistoryError>
where
    I: Iterator<Item = String>,
{
    ...
    let entry = json!({
        ...
        "deployable_unit": read_string(&handoff, "deployable_unit", args.handoff.as_path())?,
        "rollout_entrypoint": read_string(&handoff, "rollout_entrypoint", args.handoff.as_path())?,
        "launched_at_unix_ms": read_u64(&handoff, "launched_at_unix_ms", args.handoff.as_path())?,
    ...
    emit_json_artifact_written(
        "deploy_cli",
        "deployment_stack_launch_evidence_handoff_history_written",
        "history_path",
        &args.out_path,
        "history",
        history,
...
fn parse_args<I>(args: I) -> Result<Args, DeploymentStackLaunchHandoffHistoryError>
where
    I: Iterator<Item = String>,
{
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ...
            _ if arg.starts_with("--recorded-at-unix-ms=") => {
                recorded_at_unix_ms = Some(
                    arg.trim_start_matches("--recorded-at-unix-ms=")
                        .parse::<u64>()
                        .map_err(|_| {
                            DeploymentStackLaunchHandoffHistoryError::InvalidArg(format!(
            ...
            _ => {
                return Err(DeploymentStackLaunchHandoffHistoryError::InvalidArg(
                    format!(
                        "unknown argument for deploy record-launch-handoff-history: {arg}\n{}",
                        usage()
                    ),
                ));
            }
...
fn parse_value<T, I>(
    args: &mut I,
    flag: &str,
) -> Result<T, DeploymentStackLaunchHandoffHistoryError>
where
    T: std::str::FromStr,
    I: Iterator<Item = String>,
{
