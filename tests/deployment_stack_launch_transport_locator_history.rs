...
#[test]
fn deployment_stack_launch_transport_locator_history_schema_and_workflow_are_explicit() {
    let schema_text = fs::read_to_string(fixture_path(
        "deployment_stack_launch_transport_locator_history.schema.json",
    ))
    .expect("read deployment stack launch transport locator history schema");
    let schema: Value = serde_json::from_str(&schema_text)
        .expect("parse deployment stack launch transport locator history schema");
    ...
    assert!(
        justfile
            .contains("deploy-launch-transport-locator-history locator event recorded_at_unix_ms:"),
        "justfile must expose the deploy-launch-transport-locator-history workflow"
    );
    ...
    assert!(
        workflows.contains("just deploy-launch-transport-locator-history"),
        "workflow reference must mention the deployment stack launch transport locator history workflow"
    );
}

#[test]
fn deployment_stack_launch_transport_locator_history_writes_history_and_event() {
  ...
  "rollout_entrypoint": "/srv/afterburner/artifacts/inference/current"
    ...
    let out = tmp
        .path()
        .join("deployment_stack_launch_transport_locator_history.json");

    let mut cmd = cargo_bin_cmd!("afterburner");
    cmd.arg("deploy")
        .arg("record-launch-transport-locator-history")
        .arg("--locator")
        .arg(&locator)
        .arg("--event")
        .arg("pointed")
    ...
    let event = stderr
        ...
        .find(|event| {
            event.get("event").and_then(Value::as_str)
                == Some("deployment_stack_launch_transport_locator_history_written")
        })
        .unwrap_or_else(|| panic!("missing transport locator history event in stderr: {stderr}"));
    ...
    assert_eq!(
        ...
        serde_json::json!({
            ...
            "level": "info",
            "source": "deploy_cli",
            "event": "deployment_stack_launch_transport_locator_history_written",
            "fields": {
