...
- Distributed shard lineage transport locator adapter [Distributed Training, Data Infra]
  - Goal: Export one stable transport-facing locator over the distributed shard lineage evidence handoff so downstream distributed tooling can resolve the current lineage handoff without depending on local handoff path conventions.
  - Kind: `mixed`
  - Boundary: `core-contract`
  - Contracts: `artifact`, `event`
  - Scope: `src/`, `fixtures/`, `tests/`, `README.md`, `docs/adr/`

- kube-rs GPU lease reconciliation adapter [Runtime Infra, Serving/Deployment Infra]
  - Goal: Materialize a kube-rs-oriented lease reconciliation surface that reuses the existing scheduler lease and lifecycle model so the cluster path can evolve through one adapter contract instead of ad hoc controller assumptions.
  - Kind: `mixed`
  - Boundary: `adapter-deployment`
  - Contracts: `artifact`, `ops`, `event`
  - Scope: `src/`, `fixtures/`, `tests/`, `README.md`, `docs/adr/`

- Deployment stack launch locator history adapter [Serving/Deployment Infra, Runtime Infra]
  - Goal: Append one compact history record over deployment stack launch locator pointer changes so downstream deployment tooling can audit pointer updates without reading shell or process logs.
  - Kind: `mixed`
  - Boundary: `adapter-deployment`
  - Contracts: `artifact`, `event`, `ops`
  - Scope: `src/`, `fixtures/`, `tests/`, `README.md`, `docs/adr/`

- Distributed shard lineage locator pointer adapter [Distributed Training, Data Infra]
  - Goal: Export one stable current-pointer artifact over the distributed shard lineage transport locator so downstream distributed tooling can resolve the current lineage locator without depending on local locator path conventions.
  - Kind: `mixed`
  - Boundary: `core-contract`
  - Contracts: `artifact`, `event`
  - Scope: `src/`, `fixtures/`, `tests/`, `README.md`, `docs/adr/`

- kube-rs GPU lease pointer adapter [Runtime Infra, Serving/Deployment Infra]
  - Goal: Export one stable current-pointer artifact over the kube-rs GPU lease reconciliation surface so the cluster-path scheduler can resolve the active lease state without depending on local reconciliation artifact paths.
  - Kind: `mixed`
  - Boundary: `adapter-deployment`
  - Contracts: `artifact`, `ops`, `event`
  - Scope: `src/`, `fixtures/`, `tests/`, `README.md`, `docs/adr/`

- Distributed shard lineage locator history adapter [Distributed Training, Data Infra]
  - Goal: Append one compact history artifact over distributed shard lineage locator pointer changes so downstream lineage tooling can audit locator updates without reading shell or process logs.
  - Kind: `mixed`
  - Boundary: `core-contract`
  - Contracts: `artifact`, `event`
  - Scope: `src/`, `fixtures/`, `tests/`, `README.md`, `docs/adr/`
