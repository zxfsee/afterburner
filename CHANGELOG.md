...
- Profiling provenance receipt adapter [Runtime Infra, Experimentation/Eval Infra]
  - Goal: Materialize the profiling provenance receipt over the current environment snapshot and captured profiling metadata so profiling reviews can audit one explicit provenance record instead of relying on surrounding workflow state.
  - Kind: `mixed`
