...
#[test]
fn distributed_shard_lineage_evidence_bundle_reconciliation_history_schema_and_workflow_are_explicit()
 {
    let schema_text = fs::read_to_string(fixture_path(
        "distributed_shard_lineage_evidence_bundle_reconciliation_history.schema.json",
    ))
    .expect("read lineage evidence bundle reconciliation history schema");
    let schema: Value = serde_json::from_str(&schema_text)
        .expect("parse lineage evidence bundle reconciliation history schema");
    ...
    assert!(
        ...
        "justfile must expose the distributed-shard-lineage-bundle-history workflow"
    );
}

#[test]
fn distributed_shard_lineage_evidence_bundle_reconciliation_history_writes_history_and_event() {
  ...
  "reconciliation_status": "aligned"
                        ...
                        "evidence_sources": [
                            {
                                "metadata_path": "fixtures/distributed_shard_metadata.example.json",
                                "checkpoint_root": "artifacts/train/checkpoints/group-0007",
                                "observed_at_unix_ms": 1735689600000_i64
                            }
                        ]
                    },
                    "current": {
    ...
    let event = stderr
        ...
        .find(|event| {
            event.get("event").and_then(Value::as_str)
                == Some("distributed_shard_lineage_evidence_bundle_reconciliation_history_written")
        })
        .unwrap_or_else(|| {
            panic!("missing lineage bundle reconciliation history event in stderr: {stderr}")
        });
    ...
    assert_eq!(
        ...
        serde_json::json!({
            ...
            "level": "info",
            "source": "train_cli",
            "event": "distributed_shard_lineage_evidence_bundle_reconciliation_history_written",
            "fields": {
