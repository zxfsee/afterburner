...
fn load_json_object(
    path: &Path,
    kind: &str,
) -> Result<serde_json::Map<String, Value>, DriftBaselinePointerError> {
    let text = fs::read_to_string(path)?;
    let value: Value = serde_json::from_str(&text).map_err(|err| {
        DriftBaselinePointerError::Parse(format!("parse {kind} `{}`: {err}", path.display()))
    })?;
    value.as_object().cloned().ok_or_else(|| {
        DriftBaselinePointerError::Parse(format!("{kind} `{}` must be an object", path.display()))
    })
}
...
fn read_string(
    ...
    key: &'static str,
    path: &Path,
) -> Result<String, DriftBaselinePointerError> {
    object
        .get(key)
        .and_then(Value::as_str)
        .map(str::to_string)
        .ok_or_else(|| {
            DriftBaselinePointerError::Parse(format!(
