...
#[test]
fn deployment_verification_receipt_locator_pointer_rollback_schema_and_workflow_are_explicit() {
    let schema_text = fs::read_to_string(fixture_path(
        "deployment_verification_receipt_locator_pointer_rollback.schema.json",
    ))
    .expect("read deployment verification receipt locator rollback schema");
    let schema: Value = serde_json::from_str(&schema_text)
        .expect("parse deployment verification receipt locator rollback schema");

    assert_eq!(
        schema.get("$id").and_then(Value::as_str),
        Some(
            "https://afterburner.local/schemas/deployment-verification-receipt-locator-pointer-rollback/v1"
        )
    );
    let required = schema
        .get("required")
    ...
    assert!(
        ...
        "justfile must expose the deployment-verification-rollback-receipt-locator workflow"
    );

    let workflows = repo_file("docs/workflows.md");
    assert!(
        workflows.contains("deployment_verification_receipt_locator_pointer_rollback.json"),
        "workflow reference must mention the deployment verification receipt locator rollback artifact"
    );
    assert!(
        workflows.contains("just deployment-verification-rollback-receipt-locator"),
        "workflow reference must mention the deployment verification receipt locator rollback workflow"
    );
}

#[test]
fn deployment_verification_receipt_locator_pointer_rollback_writes_pointer_and_rollback_record() {
  ...
  "verification_status": "passed"
    ...
    fs::write(
        ...
        r#"{
  "schema_version": "1",
  "locator_path": "artifacts/deploy/deployment_verification_receipt_transport_locator.previous.json",
  ...
  "verification_status": "passed"
    ...
    let event = stderr
        ...
        .find(|event| {
            event.get("event").and_then(Value::as_str)
                == Some("deployment_verification_receipt_locator_pointer_rolled_back")
        })
        .unwrap_or_else(|| {
            panic!(
                "missing deployment verification receipt locator rollback event in stderr: {stderr}"
            )
        });
    ...
    assert_eq!(
        ...
        serde_json::json!({
            ...
            "level": "info",
            "source": "deploy_cli",
            "event": "deployment_verification_receipt_locator_pointer_rolled_back",
            "fields": {
