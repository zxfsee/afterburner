...
#[test]
fn deployment_verification_bundle_rollback_reconciliation_history_schema_and_workflow_are_explicit()
{
    let schema_text = fs::read_to_string(fixture_path(
        "deployment_verification_evidence_bundle_rollback_reconciliation_history.schema.json",
    ))
    .expect("read deployment verification bundle rollback reconciliation history schema");
    let schema: Value = serde_json::from_str(&schema_text)
        .expect("parse deployment verification bundle rollback reconciliation history schema");
    ...
    assert!(
        ...
        "justfile must expose the deployment-verification-record-bundle-rollback-reconciliation-history workflow"
    );
    ...
    assert!(
        ...
        "workflow reference must mention the deployment verification bundle rollback reconciliation history artifact"
    );
    assert!(
        workflows
            .contains("just deployment-verification-record-bundle-rollback-reconciliation-history"),
        "workflow reference must mention the deployment verification bundle rollback reconciliation history workflow"
    );
}

#[test]
fn deployment_verification_bundle_rollback_reconciliation_history_writes_history_and_event() {
    ...
    fs::write(
        ...
        r#"{
  "schema_version": "1",
  "rollback_path": "artifacts/deploy/deployment_verification_evidence_bundle_rollback.json",
  "current_rollback_path": "artifacts/deploy/deployment_verification_evidence_bundle_rollback.current.json",
  "desired": {
  ...
  "reconciliation_status": "aligned"
    ...
    let event = stderr
        ...
        .find(|event| {
            event.get("event").and_then(Value::as_str)
                == Some("deployment_verification_evidence_bundle_rollback_reconciliation_history_written")
        })
        .unwrap_or_else(|| {
            panic!(
                "missing deployment verification bundle rollback reconciliation history event in stderr: {stderr}"
            )
        });
    ...
    assert_eq!(
        ...
        serde_json::json!({
            ...
            "level": "info",
            "source": "deploy_cli",
            "event": "deployment_verification_evidence_bundle_rollback_reconciliation_history_written",
            "fields": {
