...
impl From<std::io::Error> for DistributedShardLineageHandoffReconciliationHistoryError {
    fn from(value: std::io::Error) -> Self {
        Self::Io(value)
    }
}

impl From<JsonArtifactError> for DistributedShardLineageHandoffReconciliationHistoryError {
...
fn load_json_object(
    path: &Path,
    kind: &str,
) -> Result<serde_json::Map<String, Value>, DistributedShardLineageHandoffReconciliationHistoryError>
{
    let text = fs::read_to_string(path)?;
    let value: Value = serde_json::from_str(&text).map_err(|err| {
        DistributedShardLineageHandoffReconciliationHistoryError::Parse(format!(
            "parse {kind} `{}`: {err}",
            path.display()
        ))
    })?;
...
fn load_history(
    path: &Path,
) -> Result<Value, DistributedShardLineageHandoffReconciliationHistoryError> {
    let text = fs::read_to_string(path)?;
    let history: Value = serde_json::from_str(&text).map_err(|err| {
        DistributedShardLineageHandoffReconciliationHistoryError::Parse(format!(
            "parse distributed shard lineage handoff reconciliation history `{}`: {err}",
            path.display()
        ))
    })?;
    history
        ...
        .ok_or_else(|| {
            DistributedShardLineageHandoffReconciliationHistoryError::Parse(format!(
                "distributed shard lineage handoff reconciliation history `{}` must contain an `entries` array",
                path.display()
            ))
        })?;
    Ok(history)
...
fn read_string(
    ...
    key: &'static str,
    path: &Path,
) -> Result<String, DistributedShardLineageHandoffReconciliationHistoryError> {
    object
        .get(key)
        ...
        .ok_or_else(|| {
            DistributedShardLineageHandoffReconciliationHistoryError::Parse(format!(
                "distributed shard lineage evidence handoff reconciliation `{}` missing string field `{key}`",
                path.display()
            ))
        })
}
...
fn read_object(
    ...
    object.get(key).cloned().ok_or_else(|| {
        DistributedShardLineageHandoffReconciliationHistoryError::Parse(format!(
            "distributed shard lineage evidence handoff reconciliation `{}` missing field `{key}`",
            path.display()
        ))
    })
}
