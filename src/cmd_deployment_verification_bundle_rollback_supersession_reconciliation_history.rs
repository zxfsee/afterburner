...
const DEFAULT_OUT_PATH: &str = "artifacts/deploy/deployment_verification_evidence_bundle_rollback_supersession_reconciliation_history.json";

#[derive(Debug)]
enum DeploymentVerificationBundleRollbackSupersessionReconciliationHistoryError {
    InvalidArg(String),
    Io(std::io::Error),
    Parse(String),
}
...
impl std::fmt::Display
    for DeploymentVerificationBundleRollbackSupersessionReconciliationHistoryError
{
    ...
    fn from(value: afterburner::command_artifacts::JsonArtifactError) -> Self {
        match value {
            ...
            afterburner::command_artifacts::JsonArtifactError::Serialize(err) => {
                Self::Parse(format!(
                    "serialize deployment verification bundle rollback supersession reconciliation history json: {err}"
                ))
            }
        }
    }
...
fn run_inner<I>(
    args: I,
) -> Result<(), DeploymentVerificationBundleRollbackSupersessionReconciliationHistoryError>
where
    I: Iterator<Item = String>,
{
    ...
    let reconciliation = load_json_object(
        ...
        "deployment verification bundle rollback supersession reconciliation",
    )?;
    ...
    let entry = json!({
        ...
        "supersession_path": read_string(&reconciliation, "supersession_path", args.reconciliation.as_path(), "deployment verification bundle rollback supersession reconciliation")?,
        "current_supersession_path": read_string(&reconciliation, "current_supersession_path", args.reconciliation.as_path(), "deployment verification bundle rollback supersession reconciliation")?,
        "reconciliation_status": read_string(&reconciliation, "reconciliation_status", args.reconciliation.as_path(), "deployment verification bundle rollback supersession reconciliation")?,
        "desired": read_object(&reconciliation, "desired", args.reconciliation.as_path(), "deployment verification bundle rollback supersession reconciliation")?,
        "current": read_object(&reconciliation, "current", args.reconciliation.as_path(), "deployment verification bundle rollback supersession reconciliation")?,
...
fn parse_args<I>(
    args: I,
) -> Result<Args, DeploymentVerificationBundleRollbackSupersessionReconciliationHistoryError>
where
    I: Iterator<Item = String>,
{
        ...
        match arg.as_str() {
            ...
            _ if arg.starts_with("--recorded-at-unix-ms=") => recorded_at_unix_ms = Some(
                arg.trim_start_matches("--recorded-at-unix-ms=")
                    .parse::<u64>()
                    .map_err(|_| {
                        DeploymentVerificationBundleRollbackSupersessionReconciliationHistoryError::InvalidArg(
            ...
            _ => {
                return Err(
                    DeploymentVerificationBundleRollbackSupersessionReconciliationHistoryError::InvalidArg(
                        format!(
                            "unknown argument for deploy record-verification-bundle-rollback-supersession-reconciliation-history: {arg}\n{}",
                            usage()
                        ),
                    ),
                );
...
fn parse_value<T, I>(
    args: &mut I,
    flag: &str,
) -> Result<T, DeploymentVerificationBundleRollbackSupersessionReconciliationHistoryError>
where
    T: std::str::FromStr,
    I: Iterator<Item = String>,
{
...
fn usage() -> &'static str {
    "usage: afterburner debug deploy record-verification-bundle-rollback-supersession-reconciliation-history --reconciliation PATH --event NAME --recorded-at-unix-ms MS [--out PATH]"
}
