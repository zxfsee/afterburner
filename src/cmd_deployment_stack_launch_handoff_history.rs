...
fn load_json_object(
    path: &Path,
    kind: &str,
) -> Result<serde_json::Map<String, Value>, DeploymentStackLaunchHandoffHistoryError> {
    let text = fs::read_to_string(path)?;
    let value: Value = serde_json::from_str(&text).map_err(|err| {
        DeploymentStackLaunchHandoffHistoryError::Parse(format!(
            "parse {kind} `{}`: {err}",
            path.display()
        ))
    })?;
...
fn load_history(path: &Path) -> Result<Value, DeploymentStackLaunchHandoffHistoryError> {
    let text = fs::read_to_string(path)?;
    let history: Value = serde_json::from_str(&text).map_err(|err| {
        DeploymentStackLaunchHandoffHistoryError::Parse(format!(
            "parse deployment stack launch handoff history `{}`: {err}",
            path.display()
        ))
    })?;
    history
        ...
        .ok_or_else(|| {
            DeploymentStackLaunchHandoffHistoryError::Parse(format!(
                "deployment stack launch handoff history `{}` must contain an `entries` array",
                path.display()
            ))
        })?;
    Ok(history)
...
fn read_string(
    ...
    key: &'static str,
    path: &Path,
) -> Result<String, DeploymentStackLaunchHandoffHistoryError> {
    object
        .get(key)
...
fn read_u64(
    ...
    key: &'static str,
    path: &Path,
) -> Result<u64, DeploymentStackLaunchHandoffHistoryError> {
    object.get(key).and_then(Value::as_u64).ok_or_else(|| {
        DeploymentStackLaunchHandoffHistoryError::Parse(format!(
            "deployment stack launch evidence handoff `{}` missing integer field `{key}`",
            path.display()
        ))
    })
}
