...
#[test]
fn deployment_verification_bundle_transport_locator_history_schema_and_workflow_are_explicit() {
    let schema_text = fs::read_to_string(fixture_path(
        "deployment_verification_evidence_bundle_transport_locator_history.schema.json",
    ))
    .expect("read deployment verification bundle transport locator history schema");
    let schema: Value = serde_json::from_str(&schema_text)
        .expect("parse deployment verification bundle transport locator history schema");
    ...
    assert!(
        ...
        "justfile must expose the deployment-verification-record-bundle-transport-locator-history workflow"
    );

    let workflows = repo_file("docs/workflows.md");
    assert!(
        workflows
            .contains("deployment_verification_evidence_bundle_transport_locator_history.json"),
        "workflow reference must mention the deployment verification bundle transport locator history artifact"
    );
    assert!(
        workflows.contains("just deployment-verification-record-bundle-transport-locator-history"),
        "workflow reference must mention the deployment verification bundle transport locator history workflow"
    );
}

#[test]
fn deployment_verification_bundle_transport_locator_history_writes_history_and_event() {
  ...
  "verification_status": "passed"
    ...
    let event = stderr
        ...
        .find(|event| {
            event.get("event").and_then(Value::as_str)
                == Some("deployment_verification_evidence_bundle_transport_locator_history_written")
        })
        .unwrap_or_else(|| {
            panic!(
                "missing deployment verification bundle transport locator history event in stderr: {stderr}"
            )
        });
    ...
    assert_eq!(
        ...
        serde_json::json!({
            ...
            "level": "info",
            "source": "deploy_cli",
            "event": "deployment_verification_evidence_bundle_transport_locator_history_written",
            "fields": {
