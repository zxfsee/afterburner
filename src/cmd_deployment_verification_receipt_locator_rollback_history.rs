...
fn load_json_object(
    path: &Path,
    kind: &str,
) -> Result<serde_json::Map<String, Value>, DeploymentVerificationReceiptLocatorRollbackHistoryError>
{
    let text = fs::read_to_string(path)?;
    let value: Value = serde_json::from_str(&text).map_err(|err| {
        DeploymentVerificationReceiptLocatorRollbackHistoryError::Parse(format!(
            "parse {kind} `{}`: {err}",
            path.display()
        ))
    })?;
...
fn load_history(
    path: &Path,
) -> Result<Value, DeploymentVerificationReceiptLocatorRollbackHistoryError> {
    let text = fs::read_to_string(path)?;
    serde_json::from_str(&text).map_err(|err| {
        DeploymentVerificationReceiptLocatorRollbackHistoryError::Parse(format!(
            "parse deployment verification receipt locator rollback history `{}`: {err}",
            path.display()
        ))
    })
}
...
fn history_entries_mut(
    history: &mut Value,
) -> Result<&mut Vec<Value>, DeploymentVerificationReceiptLocatorRollbackHistoryError> {
    history
        .get_mut("entries")
        .and_then(Value::as_array_mut)
        .ok_or_else(|| {
            DeploymentVerificationReceiptLocatorRollbackHistoryError::Parse(
                "deployment verification receipt locator rollback history must contain an `entries` array"
                    .to_string(),
            )
        })
}
...
fn read_string(
    ...
    key: &'static str,
    path: &Path,
) -> Result<String, DeploymentVerificationReceiptLocatorRollbackHistoryError> {
    object
        .get(key)
        ...
        .ok_or_else(|| {
            DeploymentVerificationReceiptLocatorRollbackHistoryError::Parse(format!(
                "deployment verification receipt locator rollback `{}` missing string field `{key}`",
                path.display()
            ))
        })
}
...
fn read_u64(
    ...
    key: &'static str,
    path: &Path,
) -> Result<u64, DeploymentVerificationReceiptLocatorRollbackHistoryError> {
    object.get(key).and_then(Value::as_u64).ok_or_else(|| {
        DeploymentVerificationReceiptLocatorRollbackHistoryError::Parse(format!(
            "deployment verification receipt locator rollback `{}` missing integer field `{key}`",
            path.display()
        ))
    })
}
