...
- Shape commit subjects for fast visual distinction: keep only the primary change dimension in the subject, move stacked artifact/history/reconciliation detail into the body, and fold purely derived changelog refreshes into the functional commit unless the refresh is independently meaningful.
...
- Keep `just` as the canonical orchestration surface. When reconciliation/history/protocol logic starts repeating across recipes, consolidate the repeated logic into a small typed helper instead of growing recipe-local protocol code.
