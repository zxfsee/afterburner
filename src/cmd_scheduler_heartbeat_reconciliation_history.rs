        ...
        match arg.as_str() {
            ...
            _ => {
                return Err(SchedulerHeartbeatReconciliationHistoryError::InvalidArg(
                    format!(
                        "unknown argument for deploy scheduler-heartbeat reconciliation-history: {arg}\n{}",
                        usage()
                    ),
                ));
            }
...
fn usage() -> &'static str {
    "usage: afterburner debug deploy scheduler-heartbeat reconciliation-history --reconciliation PATH --event NAME --recorded-at-unix-ms MS [--out PATH]"
}
