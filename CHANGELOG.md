...
- Deployment stack launch fit gate [Serving/Deployment Infra, Runtime Infra]
  - Goal: Define the next concrete launch-fit contract for the deployable Afterburner runtime stack so local and remote rollout tooling can converge on one explicit stack launch surface instead of shell-local startup assumptions.
  - Kind: `mixed`
