...
- Inference drift baseline export bundle contract [Inference, Serving/Deployment Infra]
  - Goal: Define a compact export bundle artifact for the currently approved drift baseline state so deployment-side workflows can consume one packaged baseline contract instead of pulling individual pointer and history files ad hoc.
  - Kind: `mixed`
