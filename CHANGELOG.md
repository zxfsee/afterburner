...
- Distributed parallelism profile contract [Distributed Training, Experimentation/Eval Infra]
  - Goal: Materialize a reproducible scaling-profile artifact over model size and node count that records DP, TP, PP, GAS, MBS, ZeRO stage, MFU, and memory usage for 8-GPU nodes so parallelization choices come from measured profiles instead of ad hoc tuning.
  - Kind: `mixed`
