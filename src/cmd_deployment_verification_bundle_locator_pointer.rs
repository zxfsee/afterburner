...
#[derive(Debug)]
enum DeploymentVerificationBundleLocatorPointerError {
    InvalidArg(String),
    Io(std::io::Error),
    Parse(String),
}

impl std::fmt::Display for DeploymentVerificationBundleLocatorPointerError {
...
impl std::error::Error for DeploymentVerificationBundleLocatorPointerError {}

impl From<std::io::Error> for DeploymentVerificationBundleLocatorPointerError {
...
fn run_inner<I>(args: I) -> Result<(), DeploymentVerificationBundleLocatorPointerError>
where
    I: Iterator<Item = String>,
{
    ...
    let text = serde_json::to_string_pretty(&pointer).map_err(|err| {
        DeploymentVerificationBundleLocatorPointerError::Parse(format!(
            "serialize deployment verification bundle locator pointer json: {err}"
        ))
    })?;
    ...
    emit_event(
        "info",
        "deploy_cli",
        "deployment_verification_evidence_bundle_locator_pointer_written",
        json!({
...
fn parse_args<I>(args: I) -> Result<Args, DeploymentVerificationBundleLocatorPointerError>
where
    I: Iterator<Item = String>,
{
    let mut args = args.peekable();
    let mut locator = None::<PathBuf>;
    let mut out_path = PathBuf::from(DEPLOYMENT_VERIFICATION_BUNDLE_LOCATOR_POINTER_PATH);
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ...
            _ => {
                return Err(DeploymentVerificationBundleLocatorPointerError::InvalidArg(
                    format!(
                        "unknown argument for deploy point-verification-bundle-locator: {arg}\n{}",
                        usage()
                    ),
                ));
            }
    ...
    Ok(Args {
        locator: locator.ok_or_else(|| {
            DeploymentVerificationBundleLocatorPointerError::InvalidArg(format!(
...
fn parse_value<T, I>(
    args: &mut I,
    flag: &str,
) -> Result<T, DeploymentVerificationBundleLocatorPointerError>
where
    T: std::str::FromStr,
    I: Iterator<Item = String>,
{
...
fn usage() -> &'static str {
    "usage: afterburner deploy point-verification-bundle-locator --locator PATH [--out PATH]"
}
