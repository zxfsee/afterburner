...
- Deployment verification evidence provenance receipt adapter [Serving/Deployment Infra, Experimentation/Eval Infra]
  - Goal: Materialize explicit `evidence_sources` on the deployment verification receipt so rollout verification reviews can trace decisions back to concrete eval and deploy artifacts without raw logs.
  - Kind: `mixed`
