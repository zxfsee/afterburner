...
#[test]
fn deployment_verification_bundle_rollback_history_schema_and_workflow_are_explicit() {
    let schema_text = fs::read_to_string(fixture_path(
        "deployment_verification_evidence_bundle_rollback_history.schema.json",
    ))
    .expect("read deployment verification bundle rollback history schema");
    let schema: Value = serde_json::from_str(&schema_text)
        .expect("parse deployment verification bundle rollback history schema");
...
#[test]
fn deployment_verification_bundle_rollback_history_writes_history_and_event() {
    let tmp = tempfile::tempdir().expect("tempdir");
    let rollback = tmp
        .path()
        .join("deployment_verification_evidence_bundle_rollback.json");
    fs::write(
        &rollback,
        r#"{
  "schema_version": "1",
  ...
  "rolled_back_at_unix_ms": 1735689800000
    ...
    let event = stderr
        ...
        .find(|event| {
            event.get("event").and_then(Value::as_str)
                == Some("deployment_verification_evidence_bundle_rollback_history_written")
        })
        .unwrap_or_else(|| {
            panic!(
                "missing deployment verification bundle rollback history event in stderr: {stderr}"
            )
        });
    ...
    assert_eq!(
        ...
        serde_json::json!({
            ...
            "level": "info",
            "source": "deploy_cli",
            "event": "deployment_verification_evidence_bundle_rollback_history_written",
            "fields": {
