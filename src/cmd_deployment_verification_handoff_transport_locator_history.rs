...
fn load_json_object(
    path: &Path,
    kind: &str,
) -> Result<serde_json::Map<String, Value>, DeploymentVerificationHandoffTransportLocatorHistoryError>
{
    let text = fs::read_to_string(path)?;
    let value: Value = serde_json::from_str(&text).map_err(|err| {
        DeploymentVerificationHandoffTransportLocatorHistoryError::Parse(format!(
            "parse {kind} `{}`: {err}",
            path.display()
        ))
    })?;
...
fn load_history(
    path: &Path,
) -> Result<Value, DeploymentVerificationHandoffTransportLocatorHistoryError> {
    let text = fs::read_to_string(path)?;
    serde_json::from_str(&text).map_err(|err| {
        DeploymentVerificationHandoffTransportLocatorHistoryError::Parse(format!(
            "parse deployment verification handoff transport locator history `{}`: {err}",
            path.display()
        ))
    })
}
...
fn read_string(
    ...
    key: &'static str,
    path: &Path,
) -> Result<String, DeploymentVerificationHandoffTransportLocatorHistoryError> {
    object
        .get(key)
        ...
        .ok_or_else(|| {
            DeploymentVerificationHandoffTransportLocatorHistoryError::Parse(format!(
                "deployment verification handoff transport locator `{}` missing string field `{key}`",
                path.display()
            ))
        })
}
