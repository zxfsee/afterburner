...
- Pretraining source provenance receipt adapter [Data Infra, Pre-training]
  - Goal: Materialize the pretraining source provenance receipt over the current approval receipt and reviewed source metadata so dataset reviews can audit one explicit provenance record instead of relying on surrounding workflow state.
  - Kind: `mixed`
