...
const DEFAULT_OUT_PATH: &str =
    "artifacts/train/distributed_shard_lineage_evidence_handoff_reconciliation_history.json";

#[derive(Debug)]
enum DistributedShardLineageHandoffReconciliationHistoryError {
    InvalidArg(String),
    Io(std::io::Error),
    Parse(String),
}

impl std::fmt::Display for DistributedShardLineageHandoffReconciliationHistoryError {
...
impl std::error::Error for DistributedShardLineageHandoffReconciliationHistoryError {}

impl From<afterburner::command_artifacts::JsonArtifactError>
    for DistributedShardLineageHandoffReconciliationHistoryError
{
    fn from(value: afterburner::command_artifacts::JsonArtifactError) -> Self {
        match value {
            ...
            afterburner::command_artifacts::JsonArtifactError::Serialize(err) => {
                Self::Parse(format!(
                    "serialize distributed shard lineage handoff reconciliation history json: {err}"
                ))
            }
        }
    }
...
impl From<ReconciliationError> for DistributedShardLineageHandoffReconciliationHistoryError {
...
fn run_inner<I>(args: I) -> Result<(), DistributedShardLineageHandoffReconciliationHistoryError>
where
    I: Iterator<Item = String>,
{
    ...
    let reconciliation = load_json_object(
        ...
        "distributed shard lineage evidence handoff reconciliation",
    )?;
    ...
    let entry = json!({
        ...
        "bundle_path": read_string(&reconciliation, "bundle_path", args.reconciliation.as_path(), "distributed shard lineage evidence handoff reconciliation")?,
        "handoff_path": read_string(&reconciliation, "handoff_path", args.reconciliation.as_path(), "distributed shard lineage evidence handoff reconciliation")?,
        "reconciliation_status": read_string(&reconciliation, "reconciliation_status", args.reconciliation.as_path(), "distributed shard lineage evidence handoff reconciliation")?,
        "desired": read_object(&reconciliation, "desired", args.reconciliation.as_path(), "distributed shard lineage evidence handoff reconciliation")?,
        "current": read_object(&reconciliation, "current", args.reconciliation.as_path(), "distributed shard lineage evidence handoff reconciliation")?,
    ...
    write_emitted_json(
        "train_cli",
        "distributed_shard_lineage_evidence_handoff_reconciliation_history_written",
        "history_path",
        &args.out_path,
        "history",
        history,
...
fn parse_args<I>(args: I) -> Result<Args, DistributedShardLineageHandoffReconciliationHistoryError>
where
    I: Iterator<Item = String>,
{
            ...
            _ if arg.starts_with("--recorded-at-unix-ms=") => {
                recorded_at_unix_ms = Some(
                    arg.trim_start_matches("--recorded-at-unix-ms=")
                        .parse::<u64>()
                        .map_err(|_| {
                            DistributedShardLineageHandoffReconciliationHistoryError::InvalidArg(
...
fn parse_value<T, I>(
    args: &mut I,
    flag: &str,
) -> Result<T, DistributedShardLineageHandoffReconciliationHistoryError>
where
    T: std::str::FromStr,
    I: Iterator<Item = String>,
{
