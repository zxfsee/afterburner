...
fn deployment_verification_bundle_locator_pointer_reconciliation_schema_is_explicit() {
    ...
    .expect("read deployment verification bundle locator pointer reconciliation schema");
    let schema: Value = serde_json::from_str(&schema_text)
        .expect("parse deployment verification bundle locator pointer reconciliation schema");

    assert_eq!(
        schema.get("$id").and_then(Value::as_str),
        Some(
            "https://afterburner.local/schemas/deployment-verification-evidence-bundle-locator-pointer-reconciliation/v1"
        )
    );
    let required = schema
        .get("required")
...
#[test]
fn deployment_verification_bundle_locator_pointer_reconciliation_writes_artifact_and_event() {
  ...
  "verification_status": "passed"
    ...
    .expect("read deployment verification bundle locator pointer reconciliation example");
    let mut expected: Value = serde_json::from_str(&expected_text)
        .expect("parse deployment verification bundle locator pointer reconciliation example");
    expected
        .as_object_mut()
    ...
    let event = stderr
        ...
        .find(|event| {
            event.get("event").and_then(Value::as_str)
                == Some("deployment_verification_evidence_bundle_locator_pointer_reconciliation_written")
        })
        .unwrap_or_else(|| {
            panic!(
                "missing deployment verification bundle locator pointer reconciliation event in stderr: {stderr}"
            )
        });
    ...
    assert_eq!(
        ...
        serde_json::json!({
            ...
            "level": "info",
            "source": "deploy_cli",
            "event": "deployment_verification_evidence_bundle_locator_pointer_reconciliation_written",
            "fields": {
