...
const DEFAULT_OUT_PATH: &str =
    "artifacts/deploy/gpu_scheduler_heartbeat_reconciliation_history.json";

#[derive(Debug)]
enum SchedulerHeartbeatReconciliationHistoryError {
    InvalidArg(String),
    Io(std::io::Error),
    Parse(String),
}

impl std::fmt::Display for SchedulerHeartbeatReconciliationHistoryError {
...
impl std::error::Error for SchedulerHeartbeatReconciliationHistoryError {}

impl From<afterburner::command_artifacts::JsonArtifactError>
    for SchedulerHeartbeatReconciliationHistoryError
{
    fn from(value: afterburner::command_artifacts::JsonArtifactError) -> Self {
        match value {
            ...
            afterburner::command_artifacts::JsonArtifactError::Serialize(err) => Self::Parse(
                format!("serialize scheduler heartbeat reconciliation history json: {err}"),
            ),
        }
    }
}

impl From<ReconciliationError> for SchedulerHeartbeatReconciliationHistoryError {
...
fn run_inner<I>(args: I) -> Result<(), SchedulerHeartbeatReconciliationHistoryError>
where
    I: Iterator<Item = String>,
{
    ...
    let reconciliation = load_json_object(
        ...
        "gpu scheduler heartbeat reconciliation",
    )?;
    ...
    let entry = json!({
        ...
        "heartbeat_path": read_string(&reconciliation, "heartbeat_path", args.reconciliation.as_path(), "gpu scheduler heartbeat reconciliation")?,
        "current_heartbeat_path": read_string(&reconciliation, "current_heartbeat_path", args.reconciliation.as_path(), "gpu scheduler heartbeat reconciliation")?,
        "reconciliation_status": read_string(&reconciliation, "reconciliation_status", args.reconciliation.as_path(), "gpu scheduler heartbeat reconciliation")?,
        "desired": read_object(&reconciliation, "desired", args.reconciliation.as_path(), "gpu scheduler heartbeat reconciliation")?,
        "current": read_object(&reconciliation, "current", args.reconciliation.as_path(), "gpu scheduler heartbeat reconciliation")?,
    ...
    write_emitted_json(
        "deploy_cli",
        "gpu_scheduler_heartbeat_reconciliation_history_written",
        "history_path",
        &args.out_path,
        "history",
        history,
...
fn parse_args<I>(args: I) -> Result<Args, SchedulerHeartbeatReconciliationHistoryError>
where
    I: Iterator<Item = String>,
{
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ...
            _ if arg.starts_with("--recorded-at-unix-ms=") => {
                recorded_at_unix_ms = Some(
                    arg.trim_start_matches("--recorded-at-unix-ms=")
                        .parse::<u64>()
                        .map_err(|_| {
                            SchedulerHeartbeatReconciliationHistoryError::InvalidArg(format!(
            ...
            _ => {
                return Err(SchedulerHeartbeatReconciliationHistoryError::InvalidArg(
                    format!(
                        "unknown argument for deploy record-scheduler-heartbeat-reconciliation-history: {arg}\n{}",
                        usage()
                    ),
                ));
            }
...
fn parse_value<T, I>(
    args: &mut I,
    flag: &str,
) -> Result<T, SchedulerHeartbeatReconciliationHistoryError>
where
    T: std::str::FromStr,
    I: Iterator<Item = String>,
{
...
fn usage() -> &'static str {
    "usage: afterburner debug deploy record-scheduler-heartbeat-reconciliation-history --reconciliation PATH --event NAME --recorded-at-unix-ms MS [--out PATH]"
}
