...
#[test]
fn deployment_verification_handoff_history_schema_and_workflow_are_explicit() {
    let schema_text = fs::read_to_string(fixture_path(
        "deployment_verification_evidence_handoff_history.schema.json",
    ))
    .expect("read deployment verification handoff history schema");
    let schema: Value = serde_json::from_str(&schema_text)
        .expect("parse deployment verification handoff history schema");
...
#[test]
fn deployment_verification_handoff_history_writes_history_and_event() {
  ...
  "evidence_count": 2
    ...
    let out = tmp
        .path()
        .join("deployment_verification_evidence_handoff_history.json");
    ...
    let event = stderr
        ...
        .find(|event| {
            event.get("event").and_then(Value::as_str)
                == Some("deployment_verification_evidence_handoff_history_written")
        })
        .unwrap_or_else(|| {
            panic!("missing deployment verification handoff history event in stderr: {stderr}")
        });
    ...
    assert_eq!(
        ...
        serde_json::json!({
            ...
            "level": "info",
            "source": "deploy_cli",
            "event": "deployment_verification_evidence_handoff_history_written",
            "fields": {
