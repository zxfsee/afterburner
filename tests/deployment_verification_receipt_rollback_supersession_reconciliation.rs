...
fn deployment_verification_receipt_rollback_supersession_reconciliation_schema_and_workflow_are_explicit()
 {
    ...
    .expect("read deployment verification receipt rollback supersession reconciliation schema");
    let schema: Value = serde_json::from_str(&schema_text).expect(
        "parse deployment verification receipt rollback supersession reconciliation schema",
    );

    assert_eq!(
        schema.get("$id").and_then(Value::as_str),
        Some(
            "https://afterburner.local/schemas/deployment-verification-receipt-rollback-supersession-reconciliation/v1"
        )
    );
    let required = schema
        .get("required")
...
#[test]
fn deployment_verification_receipt_rollback_supersession_reconciliation_writes_artifact_and_event()
{
  ...
  "superseded_at_unix_ms": 1735689900000
    ...
    cmd.arg("debug")
        .arg("deploy")
        .arg("reconcile-verification-receipt-rollback-supersession")
        .arg("--supersession")
        .arg(&supersession)
    ...
    .expect("read deployment verification receipt rollback supersession reconciliation example");
    let mut expected: Value = serde_json::from_str(&expected_text).expect(
        "parse deployment verification receipt rollback supersession reconciliation example",
    );
    expected
        .as_object_mut()
    ...
    let event = stderr
        ...
        .unwrap_or_else(|| {
            panic!(
                "missing deployment verification receipt rollback supersession reconciliation event in stderr: {stderr}"
            )
        });
    ...
    assert_eq!(
        ...
        serde_json::json!({
            ...
            "level": "info",
            "source": "deploy_cli",
            "event": "deployment_verification_receipt_rollback_supersession_reconciliation_written",
            "fields": {
