...
- Avoid low-leverage gate churn. Fold gates into nearby mixed work when possible, verify that a top `Kind: gate` item still fails before treating it as runnable, and stop or regroup when adjacent gate work is only refining the same contract surface without adding real leverage.
...
- Queue order is the default driver for queued work. Explicit user prioritization, queue-only edits, or a direct objective override it; when that happens, do not silently keep queue order as the active driver.
