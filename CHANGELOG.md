...
- HTTP hardening — graceful shutdown, concurrency cap, /healthz, request IDs. [Runtime Infra]
- Telemetry shaping — normalize JSON event schema + optional JSONL sink. [Runtime Infra]
- Signing-ready manifest — signature placeholders + canonicalization notes. [Inference]
- Eval acceptance threshold gate — fail if deterministic accuracy drops below baseline. [Post-training]
- HTTP response contract fixture gate — golden fixtures for single vs batch `/infer` JSON response compatibility. [Inference, Runtime Infra]
