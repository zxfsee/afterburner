...
#[test]
fn deployment_verification_receipt_history_schema_and_workflow_are_explicit() {
    let schema_text = fs::read_to_string(fixture_path(
        "deployment_verification_receipt_history.schema.json",
    ))
    .expect("read deployment verification receipt history schema");
    let schema: Value = serde_json::from_str(&schema_text)
        .expect("parse deployment verification receipt history schema");
...
#[test]
fn deployment_verification_receipt_history_writes_history_and_event() {
    ...
    entry.insert("receipt_path".to_string(), Value::from("<receipt>"));
    assert_eq!(
        history,
        serde_json::json!({
            "schema_version": "1",
    ...
    let event = stderr
        ...
        .find(|event| {
            event.get("event").and_then(Value::as_str)
                == Some("deployment_verification_receipt_history_written")
        })
        .unwrap_or_else(|| {
            panic!("missing deployment verification receipt history event in stderr: {stderr}")
        });
    ...
    assert_eq!(
        ...
        serde_json::json!({
            ...
            "level": "info",
            "source": "deploy_cli",
            "event": "deployment_verification_receipt_history_written",
            "fields": {
