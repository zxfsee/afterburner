...
const DEFAULT_OUT_PATH: &str =
    "artifacts/train/distributed_shard_lineage_transport_locator_reconciliation_history.json";

#[derive(Debug)]
enum DistributedShardLineageTransportLocatorReconciliationHistoryError {
    InvalidArg(String),
    Io(std::io::Error),
    Parse(String),
}

impl std::fmt::Display for DistributedShardLineageTransportLocatorReconciliationHistoryError {
...
impl std::error::Error for DistributedShardLineageTransportLocatorReconciliationHistoryError {}

impl From<std::io::Error> for DistributedShardLineageTransportLocatorReconciliationHistoryError {
    fn from(value: std::io::Error) -> Self {
        Self::Io(value)
    }
}

impl From<JsonArtifactError> for DistributedShardLineageTransportLocatorReconciliationHistoryError {
    fn from(value: JsonArtifactError) -> Self {
        match value {
            JsonArtifactError::Io(err) => Self::Io(err),
            JsonArtifactError::Serialize(err) => Self::Parse(format!(
                "serialize distributed shard lineage transport locator reconciliation history json: {err}"
            )),
        }
    }
}
...
fn run_inner<I>(
    args: I,
) -> Result<(), DistributedShardLineageTransportLocatorReconciliationHistoryError>
where
    I: Iterator<Item = String>,
{
...
fn parse_args<I>(
    args: I,
) -> Result<Args, DistributedShardLineageTransportLocatorReconciliationHistoryError>
where
    I: Iterator<Item = String>,
{
    ...
    Ok(Args {
        reconciliation: reconciliation.ok_or_else(|| {
            DistributedShardLineageTransportLocatorReconciliationHistoryError::InvalidArg(format!(
...
fn parse_value<T, I>(
    args: &mut I,
    flag: &str,
) -> Result<T, DistributedShardLineageTransportLocatorReconciliationHistoryError>
where
    T: std::str::FromStr,
    I: Iterator<Item = String>,
{
...
> {
    ...
    let value: Value = serde_json::from_str(&text).map_err(|err| {
        DistributedShardLineageTransportLocatorReconciliationHistoryError::Parse(format!(
            "parse {kind} `{}`: {err}",
            path.display()
        ))
    })?;
...
fn load_history(
    path: &Path,
) -> Result<Value, DistributedShardLineageTransportLocatorReconciliationHistoryError> {
    let text = fs::read_to_string(path)?;
    let history: Value = serde_json::from_str(&text).map_err(|err| {
        DistributedShardLineageTransportLocatorReconciliationHistoryError::Parse(format!(
            "parse distributed shard lineage transport locator reconciliation history `{}`: {err}",
            path.display()
        ))
    })?;
    history
        ...
        .ok_or_else(|| {
            DistributedShardLineageTransportLocatorReconciliationHistoryError::Parse(format!(
                "distributed shard lineage transport locator reconciliation history `{}` must contain an `entries` array",
                path.display()
            ))
        })?;
    Ok(history)
...
fn read_string(
    ...
    key: &'static str,
    path: &Path,
) -> Result<String, DistributedShardLineageTransportLocatorReconciliationHistoryError> {
    object
        .get(key)
        ...
        .ok_or_else(|| {
            DistributedShardLineageTransportLocatorReconciliationHistoryError::Parse(format!(
                "distributed shard lineage transport locator reconciliation `{}` missing string field `{key}`",
                path.display()
            ))
        })
}
...
fn read_object(
    ...
    object.get(key).cloned().ok_or_else(|| {
        DistributedShardLineageTransportLocatorReconciliationHistoryError::Parse(format!(
            "distributed shard lineage transport locator reconciliation `{}` missing field `{key}`",
            path.display()
        ))
    })
}
