...
- Artifact cleanup execution evidence provenance receipt [Serving/Deployment Infra, Runtime Infra]
  - Goal: Materialize explicit evidence-source references on the cleanup execution receipt so removal and skip reviews can trace each execution back to the dry-run plan and inventory/policy inputs without raw logs.
  - Kind: `mixed`
