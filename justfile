...
scheduler-heartbeat-point-supersession-reconcile supersession current_supersession:
    cargo run --locked --bin afterburner -- debug deploy reconcile-scheduler-heartbeat-pointer-supersession --supersession {{ supersession }} --current-supersession {{ current_supersession }} --out artifacts/deploy/gpu_scheduler_heartbeat_pointer_supersession_reconciliation.json

scheduler-heartbeat-point-record-supersession-reconciliation-history reconciliation event recorded_at_unix_ms:
    cargo run --locked --bin afterburner -- debug deploy record-scheduler-heartbeat-pointer-supersession-reconciliation-history --reconciliation {{ reconciliation }} --event {{ event }} --recorded-at-unix-ms {{ recorded_at_unix_ms }} --out artifacts/deploy/gpu_scheduler_heartbeat_pointer_supersession_reconciliation_history.json

scheduler-heartbeat-point-rollback current_pointer restored_pointer rolled_back_at_unix_ms:
    cargo run --locked --bin afterburner -- debug deploy rollback-scheduler-heartbeat-pointer --current-pointer {{ current_pointer }} --restored-pointer {{ restored_pointer }} --rolled-back-at-unix-ms {{ rolled_back_at_unix_ms }} --out-pointer artifacts/deploy/gpu_scheduler_heartbeat_pointer.json --out-record artifacts/deploy/gpu_scheduler_heartbeat_pointer_rollback.json

scheduler-heartbeat-point-record-rollback-history rollback event recorded_at_unix_ms:
    cargo run --locked --bin afterburner -- debug deploy record-scheduler-heartbeat-pointer-rollback-history --rollback {{ rollback }} --event {{ event }} --recorded-at-unix-ms {{ recorded_at_unix_ms }} --out artifacts/deploy/gpu_scheduler_heartbeat_pointer_rollback_history.json

scheduler-heartbeat-point-rollback-reconcile rollback current_rollback:
    cargo run --locked --bin afterburner -- debug deploy reconcile-scheduler-heartbeat-pointer-rollback --rollback {{ rollback }} --current-rollback {{ current_rollback }} --out artifacts/deploy/gpu_scheduler_heartbeat_pointer_rollback_reconciliation.json

scheduler-heartbeat-point-record-rollback-reconciliation-history reconciliation event recorded_at_unix_ms:
    cargo run --locked --bin afterburner -- debug deploy record-scheduler-heartbeat-pointer-rollback-reconciliation-history --reconciliation {{ reconciliation }} --event {{ event }} --recorded-at-unix-ms {{ recorded_at_unix_ms }} --out artifacts/deploy/gpu_scheduler_heartbeat_pointer_rollback_reconciliation_history.json
...
scheduler-heartbeat-point-record-rollback-supersession-reconciliation-history reconciliation event recorded_at_unix_ms:
    cargo run --locked --bin afterburner -- debug deploy record-scheduler-heartbeat-pointer-rollback-supersession-reconciliation-history --reconciliation {{ reconciliation }} --event {{ event }} --recorded-at-unix-ms {{ recorded_at_unix_ms }} --out artifacts/deploy/gpu_scheduler_heartbeat_pointer_rollback_supersession_reconciliation_history.json
