...
fn distributed_shard_lineage_locator_pointer_schema_and_workflow_are_explicit() {
    ...
    assert!(
        justfile.contains("distributed-shard-lineage-locator-pointer locator:"),
        "justfile must expose the grouped distributed-shard-lineage-locator pointer workflow"
    );
}
