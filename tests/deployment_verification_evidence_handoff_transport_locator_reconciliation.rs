...
fn deployment_verification_handoff_transport_locator_reconciliation_schema_and_workflow_are_explicit()
 {
    ...
    .expect("read deployment verification handoff transport locator reconciliation schema");
    let schema: Value = serde_json::from_str(&schema_text)
        .expect("parse deployment verification handoff transport locator reconciliation schema");

    assert_eq!(
        schema.get("$id").and_then(Value::as_str),
        Some(
            "https://afterburner.local/schemas/deployment-verification-evidence-handoff-transport-locator-reconciliation/v1"
        )
    );
    let required = schema
        .get("required")
...
#[test]
fn deployment_verification_handoff_transport_locator_reconciliation_writes_artifact_and_event() {
  ...
  "evidence_count": 2
    ...
    .expect("write deployment verification evidence handoff");
    let locator = tmp
        .path()
        .join("deployment_verification_evidence_handoff_transport_locator.json");
    fs::write(
        &locator,
        format!(
            r#"{{
  ...
  "verification_status": "passed"
    ...
    current.insert("handoff_path".to_string(), Value::from("<handoff>"));
    ...
    .expect("read deployment verification handoff transport locator reconciliation example");
    let mut expected: Value = serde_json::from_str(&expected_text)
        .expect("parse deployment verification handoff transport locator reconciliation example");
    expected
        .as_object_mut()
    ...
    expected_current.insert("handoff_path".to_string(), Value::from("<handoff>"));
    ...
    let event = stderr
        ...
        .unwrap_or_else(|| {
            panic!(
                "missing deployment verification handoff transport locator reconciliation event in stderr: {stderr}"
            )
        });
    ...
    reconciliation_obj.insert("handoff_path".to_string(), Value::from("<handoff>"));
    ...
    event_current.insert("handoff_path".to_string(), Value::from("<handoff>"));
    assert_eq!(
        normalized_event,
        serde_json::json!({
            "ts_ms": 0,
            "level": "info",
            "source": "deploy_cli",
            "event": "deployment_verification_evidence_handoff_transport_locator_reconciliation_written",
            "fields": {
