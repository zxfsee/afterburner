...
- kube-rs GPU lease history adapter [Runtime Infra, Serving/Deployment Infra]
  - Goal: Append one compact history artifact over kube-rs GPU lease pointer changes so cluster-path scheduler tooling can audit lease-state updates without reading controller or shell logs.
  - Kind: `mixed`
  - Boundary: `adapter-deployment`
  - Contracts: `artifact`, `ops`, `event`
  - Scope: `src/`, `fixtures/`, `tests/`, `README.md`, `docs/adr/`

- Deployment stack launch locator reconciliation adapter [Serving/Deployment Infra, Runtime Infra]
  - Goal: Materialize one reconciliation artifact over the deployment stack launch locator pointer so downstream deployment consumers can compare desired versus current locator state without depending on ad hoc shell checks.
  - Kind: `mixed`
  - Boundary: `adapter-deployment`
  - Contracts: `artifact`, `ops`, `event`
  - Scope: `src/`, `fixtures/`, `tests/`, `README.md`, `docs/adr/`

- Deployment stack launch handoff history adapter [Serving/Deployment Infra, Runtime Infra]
  - Goal: Append one compact history artifact over deployment stack launch evidence handoff changes so downstream deployment tooling can audit handoff updates without reading shell or process logs.
  - Kind: `mixed`
  - Boundary: `adapter-deployment`
