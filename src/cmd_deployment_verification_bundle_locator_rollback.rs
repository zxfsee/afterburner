...
fn load_json_object(
    path: &Path,
    kind: &str,
) -> Result<serde_json::Map<String, Value>, DeploymentVerificationBundleLocatorRollbackError> {
    let text = fs::read_to_string(path)?;
    let value: Value = serde_json::from_str(&text).map_err(|err| {
        DeploymentVerificationBundleLocatorRollbackError::Parse(format!(
            "parse {kind} `{}`: {err}",
            path.display()
        ))
    })?;
...
fn read_string(
    ...
    path: &Path,
    kind: &str,
) -> Result<String, DeploymentVerificationBundleLocatorRollbackError> {
    object
        .get(key)
...
fn write_json(
    ...
    value: &Value,
    kind: &str,
) -> Result<(), DeploymentVerificationBundleLocatorRollbackError> {
...
fn usage() -> &'static str {
    "usage: afterburner deploy rollback-verification-bundle-locator --current-pointer PATH --restored-pointer PATH --rolled-back-at-unix-ms N [--out-pointer PATH] [--out-record PATH]"
}
