...
#[test]
fn deployment_verification_evidence_handoff_reconciliation_schema_and_workflow_are_explicit() {
    let schema_text = fs::read_to_string(fixture_path(
        "deployment_verification_evidence_handoff_reconciliation.schema.json",
    ))
    .expect("read deployment verification evidence handoff reconciliation schema");
    let schema: Value = serde_json::from_str(&schema_text)
        .expect("parse deployment verification evidence handoff reconciliation schema");

    assert_eq!(
        schema.get("$id").and_then(Value::as_str),
        Some(
            "https://afterburner.local/schemas/deployment-verification-evidence-handoff-reconciliation/v1"
        )
    );
    let required = schema
        .get("required")
...
#[test]
fn deployment_verification_evidence_handoff_reconciliation_writes_artifact_and_event() {
  ...
  "evidence_count": 2
    ...
    .expect("read deployment verification evidence handoff reconciliation example");
    let mut expected: Value = serde_json::from_str(&expected_text)
        .expect("parse deployment verification evidence handoff reconciliation example");
    expected
        .as_object_mut()
    ...
    let event = stderr
        ...
        .find(|event| {
            event.get("event").and_then(Value::as_str)
                == Some("deployment_verification_evidence_handoff_reconciliation_written")
        })
        .unwrap_or_else(|| {
            panic!(
                "missing deployment verification evidence handoff reconciliation event in stderr: {stderr}"
            )
        });
    ...
    assert_eq!(
        ...
        serde_json::json!({
            ...
            "level": "info",
            "source": "deploy_cli",
            "event": "deployment_verification_evidence_handoff_reconciliation_written",
            "fields": {
