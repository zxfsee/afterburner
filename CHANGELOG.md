...
- Distributed shard lineage evidence provenance receipt adapter [Distributed Training, Data Infra]
  - Goal: Materialize explicit `evidence_sources` on the distributed shard lineage receipt so lineage reviews can trace each verified shard back to shard metadata and checkpoint roots without raw logs.
  - Kind: `mixed`
