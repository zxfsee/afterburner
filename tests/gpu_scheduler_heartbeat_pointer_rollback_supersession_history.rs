...
#[test]
fn gpu_scheduler_heartbeat_pointer_rollback_supersession_history_schema_and_workflow_are_explicit()
{
    ...
    assert!(
        ...
        "justfile must expose the scheduler-heartbeat-point-record-rollback-supersession-history workflow"
    );

    let workflows = repo_file("docs/workflows.md");
    assert!(
        workflows.contains("gpu_scheduler_heartbeat_pointer_rollback_supersession_history.json"),
        "workflow reference must mention the scheduler heartbeat pointer rollback supersession history artifact"
    );
    assert!(
        workflows.contains("just scheduler-heartbeat-point-record-rollback-supersession-history"),
        "workflow reference must mention the scheduler heartbeat pointer rollback supersession history workflow"
    );

    let reference = repo_file("docs/reference.md");
    assert!(
        reference.contains("gpu_scheduler_heartbeat_pointer_rollback_supersession_history.json"),
        "reference index must mention the scheduler heartbeat pointer rollback supersession history artifact"
    );
}
...
fn gpu_scheduler_heartbeat_pointer_rollback_supersession_history_writes_history_and_event() {
  ...
  "superseded_at_unix_ms": 1735690200000
    ...
    cmd.arg("debug")
        .arg("deploy")
        .arg("record-scheduler-heartbeat-pointer-rollback-supersession-history")
        .arg("--supersession")
        .arg(&supersession)
        .arg("--event")
        .arg("superseded")
