...
fn gpu_scheduler_heartbeat_reconciliation_schema_and_workflow_are_explicit() {
    ...
    assert!(
        justfile.contains("scheduler-heartbeat-reconcile heartbeat current_heartbeat:"),
        "justfile must expose the scheduler-heartbeat-reconcile workflow"
    );

    let workflows = repo_file("docs/workflows.md");
    assert!(
        workflows.contains("gpu_scheduler_heartbeat_reconciliation.json"),
        "workflow reference must mention the scheduler heartbeat reconciliation artifact"
    );
    assert!(
        workflows.contains("just scheduler-heartbeat-reconcile"),
        "workflow reference must mention the scheduler heartbeat reconciliation workflow"
    );

    let reference = repo_file("docs/reference.md");
    assert!(
        reference.contains("gpu_scheduler_heartbeat_reconciliation.json"),
        "reference index must mention the scheduler heartbeat reconciliation artifact"
    );
}
