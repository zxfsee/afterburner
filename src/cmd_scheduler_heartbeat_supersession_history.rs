        ...
        match arg.as_str() {
            ...
            _ => {
                return Err(SchedulerHeartbeatSupersessionHistoryError::InvalidArg(
                    format!(
                        "unknown argument for deploy scheduler-heartbeat supersession history: {arg}\n{}",
                        usage()
                    ),
                ));
            }
...
fn usage() -> &'static str {
    "usage: afterburner debug deploy scheduler-heartbeat supersession history --supersession PATH --event NAME --recorded-at-unix-ms MS [--out PATH]"
}
