...
fn gpu_scheduler_heartbeat_pointer_rollback_reconciliation_history_schema_is_explicit() {
    ...
    .expect("read pointer rollback reconciliation history schema");
    let schema: Value = serde_json::from_str(&schema_text)
        .expect("parse pointer rollback reconciliation history schema");
...
#[test]
fn gpu_scheduler_heartbeat_pointer_rollback_reconciliation_history_writes_history_and_event() {
    ...
    fs::write(
        ...
        r#"{
  "schema_version": "1",
  "rollback_path": "artifacts/deploy/gpu_scheduler_heartbeat_pointer_rollback.json",
  "current_rollback_path": "artifacts/deploy/gpu_scheduler_heartbeat_pointer_rollback.current.json",
  "desired": {
  ...
  "reconciliation_status": "aligned"
    ...
    let out = tmp
        .path()
        .join("gpu_scheduler_heartbeat_pointer_rollback_reconciliation_history.json");

    let mut cmd = cargo_bin_cmd!("afterburner");
    cmd.arg("debug")
        .arg("deploy")
        .arg("record-scheduler-heartbeat-pointer-rollback-reconciliation-history")
        .arg("--reconciliation")
        .arg(&reconciliation)
        .arg("--event")
        .arg("reconciled")
    ...
    let event = stderr
        ...
        .find(|event| {
            event.get("event").and_then(Value::as_str)
                == Some("gpu_scheduler_heartbeat_pointer_rollback_reconciliation_history_written")
        })
        .unwrap_or_else(|| {
            panic!("missing pointer rollback reconciliation history event in stderr: {stderr}")
        });
    ...
    assert_eq!(
        ...
        serde_json::json!({
            ...
            "level": "info",
            "source": "deploy_cli",
            "event": "gpu_scheduler_heartbeat_pointer_rollback_reconciliation_history_written",
            "fields": {
