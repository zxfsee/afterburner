...
#[test]
fn gpu_scheduler_heartbeat_pointer_supersession_reconciliation_schema_and_workflow_are_explicit() {
    let schema_text = fs::read_to_string(fixture_path(
        "gpu_scheduler_heartbeat_pointer_supersession_reconciliation.schema.json",
    ))
    .expect("read heartbeat pointer supersession reconciliation schema");
    let schema: Value = serde_json::from_str(&schema_text)
        .expect("parse heartbeat pointer supersession reconciliation schema");

    assert_eq!(
        schema.get("$id").and_then(Value::as_str),
        Some(
            "https://afterburner.local/schemas/gpu-scheduler-heartbeat-pointer-supersession-reconciliation/v1"
        )
    );
    let required = schema
        .get("required")
    ...
    assert!(
        ...
        "justfile must expose the scheduler-heartbeat-point-supersession-reconcile workflow"
    );

    let workflows = repo_file("docs/workflows.md");
    assert!(
        workflows.contains("gpu_scheduler_heartbeat_pointer_supersession_reconciliation.json"),
        "workflow reference must mention the scheduler heartbeat pointer supersession reconciliation artifact"
    );
    assert!(
        workflows.contains("just scheduler-heartbeat-point-supersession-reconcile"),
        "workflow reference must mention the scheduler heartbeat pointer supersession reconciliation workflow"
    );

    let reference = repo_file("docs/reference.md");
    assert!(
        reference.contains("gpu_scheduler_heartbeat_pointer_supersession_reconciliation.json"),
        "reference index must mention the scheduler heartbeat pointer supersession reconciliation artifact"
    );
}

#[test]
fn gpu_scheduler_heartbeat_pointer_supersession_reconciliation_writes_artifact_and_event() {
    ...
    let current = tmp
        .path()
        .join("gpu_scheduler_heartbeat_pointer_supersession.current.json");
    let supersession_text = r#"{
  "schema_version": "1",
  "previous_pointer_path": "artifacts/deploy/gpu_scheduler_heartbeat_pointer.previous.json",
  "next_pointer_path": "artifacts/deploy/gpu_scheduler_heartbeat_pointer.json",
  ...
  "superseded_at_unix_ms": 1735689999000
    ...
    cmd.arg("debug")
        .arg("deploy")
        .arg("reconcile-scheduler-heartbeat-pointer-supersession")
        .arg("--supersession")
        .arg(&supersession)
    ...
    .expect("read heartbeat pointer supersession reconciliation example");
    let mut expected: Value = serde_json::from_str(&expected_text)
        .expect("parse heartbeat pointer supersession reconciliation example");
    expected
        .as_object_mut()
    ...
    let event = stderr
        ...
        .find(|event| {
            event.get("event").and_then(Value::as_str)
                == Some("gpu_scheduler_heartbeat_pointer_supersession_reconciliation_written")
        })
        .unwrap_or_else(|| {
            panic!("missing pointer supersession reconciliation event in stderr: {stderr}")
        });
    ...
    assert_eq!(
        ...
        serde_json::json!({
            ...
            "level": "info",
            "source": "deploy_cli",
            "event": "gpu_scheduler_heartbeat_pointer_supersession_reconciliation_written",
            "fields": {
