...
fn distributed_shard_lineage_transport_locator_reconciliation_schema_and_workflow_are_explicit() {
    ...
    assert!(
        justfile.contains("distributed-shard-lineage-transport-locator-reconcile handoff locator:"),
        "justfile must expose the grouped distributed-shard-lineage-transport-locator reconcile workflow"
    );
}
