...
#[test]
fn gpu_scheduler_heartbeat_pointer_supersession_history_schema_and_workflow_are_explicit() {
    let schema_text = fs::read_to_string(fixture_path(
        "gpu_scheduler_heartbeat_pointer_supersession_history.schema.json",
    ))
    .expect("read heartbeat pointer supersession history schema");
    let schema: Value = serde_json::from_str(&schema_text)
        .expect("parse heartbeat pointer supersession history schema");
    ...
    assert!(
        ...
        "justfile must expose the scheduler-heartbeat-point-record-supersession-history workflow"
    );

    let workflows = repo_file("docs/workflows.md");
    assert!(
        workflows.contains("gpu_scheduler_heartbeat_pointer_supersession_history.json"),
        "workflow reference must mention the scheduler heartbeat pointer supersession history artifact"
    );
    assert!(
        workflows.contains("just scheduler-heartbeat-point-record-supersession-history"),
        "workflow reference must mention the scheduler heartbeat pointer supersession history workflow"
    );

    let reference = repo_file("docs/reference.md");
    assert!(
        reference.contains("gpu_scheduler_heartbeat_pointer_supersession_history.json"),
        "reference index must mention the scheduler heartbeat pointer supersession history artifact"
    );
}

#[test]
fn gpu_scheduler_heartbeat_pointer_supersession_history_writes_history_and_event() {
  ...
  "superseded_at_unix_ms": 1735689999000
    ...
    let out = tmp
        .path()
        .join("gpu_scheduler_heartbeat_pointer_supersession_history.json");

    let mut cmd = cargo_bin_cmd!("afterburner");
    cmd.arg("debug")
        .arg("deploy")
        .arg("record-scheduler-heartbeat-pointer-supersession-history")
        .arg("--supersession")
        .arg(&supersession)
        .arg("--event")
        .arg("superseded")
            ...
            "entries": [
                {
                    ...
                    "recorded_at_unix_ms": 1735690000000_u64
                }
            ]
        })
    );
    ...
    let event = stderr
        ...
        .find(|event| {
            event.get("event").and_then(Value::as_str)
                == Some("gpu_scheduler_heartbeat_pointer_supersession_history_written")
        })
        .unwrap_or_else(|| {
            panic!("missing pointer supersession history event in stderr: {stderr}")
        });
    ...
    assert_eq!(
        ...
        serde_json::json!({
            ...
            "level": "info",
            "source": "deploy_cli",
            "event": "gpu_scheduler_heartbeat_pointer_supersession_history_written",
            "fields": {
