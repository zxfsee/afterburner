...
#[test]
fn deployment_verification_bundle_rollback_supersession_schema_and_workflow_are_explicit() {
    ...
    assert!(
        ...
        "justfile must expose the deployment-verification-supersede-bundle-rollback workflow"
    );

    let workflows = repo_file("docs/workflows.md");
    assert!(
        workflows.contains("deployment_verification_evidence_bundle_rollback_supersession.json"),
        "workflow reference must mention the deployment verification bundle rollback supersession artifact"
    );
    assert!(
        workflows.contains("just deployment-verification-supersede-bundle-rollback"),
        "workflow reference must mention the deployment verification bundle rollback supersession workflow"
    );
}
