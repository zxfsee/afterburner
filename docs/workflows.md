...
- `just queue-execute-preflight` is the canonical execute-start path: it delegates queue freshness checks, optional stale-snapshot repair, execute pinning, and worktree validation to the typed queue preflight helper before implementation continues.
...
- `just queue-execute-preflight --repair-stale-snapshot` keeps the same surface but runs the typed stale-lineage repair path first, then continues the normal preflight while carrying the repaired `CHANGELOG.md` baseline into execute pinning unchanged.
