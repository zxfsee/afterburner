...
fn load_json_object(
    path: &Path,
    kind: &str,
) -> Result<serde_json::Map<String, Value>, DeploymentVerificationReceiptTransportLocatorError> {
...
fn read_string(
    ...
    key: &'static str,
    path: &Path,
) -> Result<String, DeploymentVerificationReceiptTransportLocatorError> {
    object
        .get(key)
...
fn usage() -> &'static str {
    "usage: afterburner deploy point-verification-receipt-transport-locator --receipt PATH [--out PATH]"
}
