...
#[test]
fn deployment_verification_bundle_reconciliation_history_schema_and_workflow_are_explicit() {
    let schema_text = fs::read_to_string(fixture_path(
        "deployment_verification_evidence_bundle_reconciliation_history.schema.json",
    ))
    .expect("read deployment verification bundle reconciliation history schema");
    let schema: Value = serde_json::from_str(&schema_text)
        .expect("parse deployment verification bundle reconciliation history schema");
...
#[test]
fn deployment_verification_bundle_reconciliation_history_writes_history_and_event() {
  ...
  "reconciliation_status": "aligned"
    ...
    let out = tmp
        .path()
        .join("deployment_verification_evidence_bundle_reconciliation_history.json");

    let mut cmd = cargo_bin_cmd!("afterburner");
    cmd.arg("deploy")
        .arg("record-verification-bundle-reconciliation-history")
        .arg("--reconciliation")
        .arg(&reconciliation)
        .arg("--event")
        .arg("reconciled")
    ...
    let event = stderr
        ...
        .find(|event| {
            event.get("event").and_then(Value::as_str)
                == Some("deployment_verification_evidence_bundle_reconciliation_history_written")
        })
        .unwrap_or_else(|| {
            panic!(
                "missing deployment verification bundle reconciliation history event in stderr: {stderr}"
            )
        });
    ...
    assert_eq!(
        ...
        serde_json::json!({
            ...
            "level": "info",
            "source": "deploy_cli",
            "event": "deployment_verification_evidence_bundle_reconciliation_history_written",
            "fields": {
