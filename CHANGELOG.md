...
- Inference drift baseline snapshot contract [Inference, Experimentation/Eval Infra]
  - Goal: Define a checked baseline snapshot artifact for infer-output drift receipts so promotion decisions compare against one approved reference instead of whichever current summary happens to be on disk.
  - Kind: `mixed`
