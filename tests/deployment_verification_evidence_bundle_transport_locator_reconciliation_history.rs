...
#[test]
fn deployment_verification_bundle_transport_locator_reconciliation_history_schema_and_workflow_are_explicit()
 {
    let schema_text = fs::read_to_string(fixture_path(
        "deployment_verification_evidence_bundle_transport_locator_reconciliation_history.schema.json",
    ))
    .expect("read deployment verification bundle transport locator reconciliation history schema");
    let schema: Value = serde_json::from_str(&schema_text).expect(
        "parse deployment verification bundle transport locator reconciliation history schema",
    );
    ...
    assert!(
        ...
        "justfile must expose the deployment-verification-record-bundle-transport-locator-reconciliation-history workflow"
    );
    ...
    assert!(
        ...
        "workflow reference must mention the deployment verification bundle transport locator reconciliation history artifact"
    );
    assert!(
        workflows.contains(
            "just deployment-verification-record-bundle-transport-locator-reconciliation-history"
        ),
        "workflow reference must mention the deployment verification bundle transport locator reconciliation history workflow"
    );
}

#[test]
fn deployment_verification_bundle_transport_locator_reconciliation_history_writes_history_and_event()
 {
  ...
  "reconciliation_status": "aligned"
    ...
    let event = stderr
        ...
        .unwrap_or_else(|| {
            panic!(
                "missing deployment verification bundle transport locator reconciliation history event in stderr: {stderr}"
            )
        });
    ...
    assert_eq!(
        ...
        serde_json::json!({
            ...
            "level": "info",
            "source": "deploy_cli",
            "event": "deployment_verification_evidence_bundle_transport_locator_reconciliation_history_written",
            "fields": {
