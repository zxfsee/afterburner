...
fn load_json_object(
    path: &Path,
    kind: &str,
) -> Result<serde_json::Map<String, Value>, DeploymentStackLaunchLocatorHistoryError> {
...
fn load_history(path: &Path) -> Result<Value, DeploymentStackLaunchLocatorHistoryError> {
    let text = fs::read_to_string(path)?;
    let value: Value = serde_json::from_str(&text).map_err(|err| {
        DeploymentStackLaunchLocatorHistoryError::Parse(format!(
            "parse deployment stack launch locator history `{}`: {err}",
            path.display()
        ))
    })?;
    if value.get("schema_version").and_then(Value::as_str) != Some("1") {
        return Err(DeploymentStackLaunchLocatorHistoryError::Parse(format!(
            "deployment stack launch locator history `{}` must use schema_version `1`",
            path.display()
        )));
    }
    if value.get("entries").and_then(Value::as_array).is_none() {
        return Err(DeploymentStackLaunchLocatorHistoryError::Parse(format!(
            "deployment stack launch locator history `{}` must include an array `entries` field",
            path.display()
        )));
    }
    Ok(value)
...
fn read_string(
    ...
    key: &'static str,
    path: &Path,
) -> Result<String, DeploymentStackLaunchLocatorHistoryError> {
    object
        .get(key)
        ...
        .ok_or_else(|| {
            DeploymentStackLaunchLocatorHistoryError::Parse(format!(
                "deployment stack launch locator pointer `{}` missing string field `{key}`",
                path.display()
            ))
        })
}
