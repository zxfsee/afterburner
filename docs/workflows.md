...
- `just queue-resume` is the explicit stale-lineage repair-and-continue path: refresh the stamped queue snapshot, rerun the horizon and completion-boundary checks, repin the execute objective, then continue execute preflight.
...
- `just queue-execute-preflight --repair-stale-snapshot` keeps the same surface but performs the explicit stale-lineage repair first: refresh the queue snapshot, rerun the horizon and completion-boundary checks, then continue the normal preflight.
