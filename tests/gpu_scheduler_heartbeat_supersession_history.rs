...
fn gpu_scheduler_heartbeat_supersession_history_schema_and_workflow_are_explicit() {
    ...
    assert!(
        ...
        "justfile must expose the scheduler-heartbeat-record-supersession-history workflow"
    );

    let workflows = repo_file("docs/workflows.md");
    assert!(
        workflows.contains("gpu_scheduler_heartbeat_supersession_history.json"),
        "workflow reference must mention the scheduler heartbeat supersession history artifact"
    );
    assert!(
        workflows.contains("just scheduler-heartbeat-record-supersession-history"),
        "workflow reference must mention the scheduler heartbeat supersession history workflow"
    );

    let reference = repo_file("docs/reference.md");
    assert!(
        reference.contains("gpu_scheduler_heartbeat_supersession_history.json"),
        "reference index must mention the scheduler heartbeat supersession history artifact"
    );
}
