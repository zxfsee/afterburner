...
fn load_json_object(
    path: &Path,
    kind: &str,
) -> Result<serde_json::Map<String, Value>, DriftBaselineHistoryError> {
    let text = fs::read_to_string(path)?;
    let value: Value = serde_json::from_str(&text).map_err(|err| {
        DriftBaselineHistoryError::Parse(format!("parse {kind} `{}`: {err}", path.display()))
    })?;
    value.as_object().cloned().ok_or_else(|| {
        DriftBaselineHistoryError::Parse(format!("{kind} `{}` must be an object", path.display()))
    })
}

fn load_history(path: &Path) -> Result<Value, DriftBaselineHistoryError> {
    let text = fs::read_to_string(path)?;
    let value: Value = serde_json::from_str(&text).map_err(|err| {
        DriftBaselineHistoryError::Parse(format!(
            "parse baseline history `{}`: {err}",
            path.display()
        ))
    })?;
    if value.get("schema_version").and_then(Value::as_str) != Some("1") {
        return Err(DriftBaselineHistoryError::Parse(format!(
            "baseline history `{}` must use schema_version `1`",
            path.display()
        )));
    }
...
fn read_string(
    ...
    key: &'static str,
    path: &Path,
) -> Result<String, DriftBaselineHistoryError> {
    object
        .get(key)
        ...
        .ok_or_else(|| {
            DriftBaselineHistoryError::Parse(format!(
                "pointer `{}` missing string field `{key}`",
                path.display()
            ))
        })
}
