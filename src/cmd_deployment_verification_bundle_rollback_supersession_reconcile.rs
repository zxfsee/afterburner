...
const DEFAULT_OUT_PATH: &str = "artifacts/deploy/deployment_verification_evidence_bundle_rollback_supersession_reconciliation.json";

#[derive(Debug)]
enum DeploymentVerificationBundleRollbackSupersessionReconcileError {
    InvalidArg(String),
    Io(std::io::Error),
    Parse(String),
}

impl std::fmt::Display for DeploymentVerificationBundleRollbackSupersessionReconcileError {
...
impl std::error::Error for DeploymentVerificationBundleRollbackSupersessionReconcileError {}

impl From<afterburner::command_artifacts::JsonArtifactError>
    for DeploymentVerificationBundleRollbackSupersessionReconcileError
{
    fn from(value: afterburner::command_artifacts::JsonArtifactError) -> Self {
        match value {
            ...
            afterburner::command_artifacts::JsonArtifactError::Serialize(err) => {
                Self::Parse(format!(
                    "serialize deployment verification bundle rollback supersession reconciliation json: {err}"
                ))
            }
        }
    }
...
impl From<ReconciliationError> for DeploymentVerificationBundleRollbackSupersessionReconcileError {
...
fn run_inner<I>(
    args: I,
) -> Result<(), DeploymentVerificationBundleRollbackSupersessionReconcileError>
where
    I: Iterator<Item = String>,
{
    ...
    let current = json!({
        ...
        "previous_rollback_path": read_string(&current_supersession, "previous_rollback_path", args.current_supersession.as_path(), "deployment verification bundle rollback supersession")?,
        "next_rollback_path": read_string(&current_supersession, "next_rollback_path", args.current_supersession.as_path(), "deployment verification bundle rollback supersession")?,
        "previous_restored_bundle_path": read_string(&current_supersession, "previous_restored_bundle_path", args.current_supersession.as_path(), "deployment verification bundle rollback supersession")?,
        "next_restored_bundle_path": read_string(&current_supersession, "next_restored_bundle_path", args.current_supersession.as_path(), "deployment verification bundle rollback supersession")?,
        "previous_restored_artifact_version": read_string(&current_supersession, "previous_restored_artifact_version", args.current_supersession.as_path(), "deployment verification bundle rollback supersession")?,
        "next_restored_artifact_version": read_string(&current_supersession, "next_restored_artifact_version", args.current_supersession.as_path(), "deployment verification bundle rollback supersession")?,
        "profile_name": read_string(&current_supersession, "profile_name", args.current_supersession.as_path(), "deployment verification bundle rollback supersession")?,
        "superseded_at_unix_ms": read_u64(&current_supersession, "superseded_at_unix_ms", args.current_supersession.as_path(), "deployment verification bundle rollback supersession")?,
    });
    let reconciliation_status = if desired["previous_rollback_path"]
        ...
        && desired["previous_restored_bundle_path"] == current["previous_restored_bundle_path"]
        && desired["next_restored_bundle_path"] == current["next_restored_bundle_path"]
...
fn parse_args<I>(
    args: I,
) -> Result<Args, DeploymentVerificationBundleRollbackSupersessionReconcileError>
where
    I: Iterator<Item = String>,
{
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ...
            _ => {
                return Err(
                    DeploymentVerificationBundleRollbackSupersessionReconcileError::InvalidArg(
                        format!(
                            "unknown argument for deploy reconcile-verification-bundle-rollback-supersession: {arg}\n{}",
                            usage()
                        ),
                    ),
                );
    ...
    Ok(Args {
        supersession: supersession.ok_or_else(|| {
            DeploymentVerificationBundleRollbackSupersessionReconcileError::InvalidArg(format!(
...
fn parse_value<T, I>(
    args: &mut I,
    flag: &str,
) -> Result<T, DeploymentVerificationBundleRollbackSupersessionReconcileError>
where
    T: std::str::FromStr,
    I: Iterator<Item = String>,
{
...
fn usage() -> &'static str {
    "usage: afterburner debug deploy reconcile-verification-bundle-rollback-supersession --supersession PATH --current-supersession PATH [--out PATH]"
}
