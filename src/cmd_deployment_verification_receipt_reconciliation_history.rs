...
const DEFAULT_OUT_PATH: &str =
    "artifacts/deploy/deployment_verification_receipt_reconciliation_history.json";

#[derive(Debug)]
enum DeploymentVerificationReceiptReconciliationHistoryError {
    InvalidArg(String),
    Io(std::io::Error),
    Parse(String),
}

impl std::fmt::Display for DeploymentVerificationReceiptReconciliationHistoryError {
...
impl std::error::Error for DeploymentVerificationReceiptReconciliationHistoryError {}

impl From<afterburner::command_artifacts::JsonArtifactError>
    for DeploymentVerificationReceiptReconciliationHistoryError
{
    fn from(value: afterburner::command_artifacts::JsonArtifactError) -> Self {
        match value {
            ...
            afterburner::command_artifacts::JsonArtifactError::Serialize(err) => {
                Self::Parse(format!(
                    "serialize deployment verification receipt reconciliation history json: {err}"
                ))
            }
        }
    }
...
impl From<ReconciliationError> for DeploymentVerificationReceiptReconciliationHistoryError {
...
fn run_inner<I>(args: I) -> Result<(), DeploymentVerificationReceiptReconciliationHistoryError>
where
    I: Iterator<Item = String>,
{
    ...
    let reconciliation = load_json_object(
        ...
        "deployment verification receipt reconciliation",
    )?;
    ...
    let entry = json!({
        ...
        "receipt_path": read_string(&reconciliation, "receipt_path", args.reconciliation.as_path(), "deployment verification receipt reconciliation")?,
        "reconciliation_status": read_string(&reconciliation, "reconciliation_status", args.reconciliation.as_path(), "deployment verification receipt reconciliation")?,
        "desired": read_object(&reconciliation, "desired", args.reconciliation.as_path(), "deployment verification receipt reconciliation")?,
        "current": read_object(&reconciliation, "current", args.reconciliation.as_path(), "deployment verification receipt reconciliation")?,
    ...
    write_emitted_json(
        "deploy_cli",
        "deployment_verification_receipt_reconciliation_history_written",
        "history_path",
        &args.out_path,
        "history",
        history,
...
fn parse_args<I>(args: I) -> Result<Args, DeploymentVerificationReceiptReconciliationHistoryError>
where
    I: Iterator<Item = String>,
{
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ...
            _ if arg.starts_with("--recorded-at-unix-ms=") => {
                recorded_at_unix_ms = Some(
                    arg.trim_start_matches("--recorded-at-unix-ms=")
                        .parse::<u64>()
                        .map_err(|_| {
                            DeploymentVerificationReceiptReconciliationHistoryError::InvalidArg(
            ...
            _ => {
                return Err(
                    DeploymentVerificationReceiptReconciliationHistoryError::InvalidArg(format!(
                        "unknown argument for deploy record-verification-receipt-reconciliation-history: {arg}\n{}",
                        usage()
                    )),
                );
            }
...
fn parse_value<T, I>(
    args: &mut I,
    flag: &str,
) -> Result<T, DeploymentVerificationReceiptReconciliationHistoryError>
where
    T: std::str::FromStr,
    I: Iterator<Item = String>,
{
...
fn usage() -> &'static str {
    "usage: afterburner debug deploy record-verification-receipt-reconciliation-history --reconciliation PATH --event NAME --recorded-at-unix-ms MS [--out PATH]"
}
