...
#[derive(Debug)]
enum DeploymentStackLaunchHandoffReconciliationHistoryError {
    InvalidArg(String),
    Io(std::io::Error),
    Parse(String),
}

impl std::fmt::Display for DeploymentStackLaunchHandoffReconciliationHistoryError {
...
fn run_inner<I>(args: I) -> Result<(), DeploymentStackLaunchHandoffReconciliationHistoryError>
where
    I: Iterator<Item = String>,
{
...
fn parse_args<I>(args: I) -> Result<Args, DeploymentStackLaunchHandoffReconciliationHistoryError>
where
    I: Iterator<Item = String>,
{
            ...
            _ if arg.starts_with("--recorded-at-unix-ms=") => {
                recorded_at_unix_ms = Some(
                    arg.trim_start_matches("--recorded-at-unix-ms=")
                        .parse::<u64>()
                        .map_err(|_| {
                            DeploymentStackLaunchHandoffReconciliationHistoryError::InvalidArg(
            ...
            _ => {
                return Err(
                    DeploymentStackLaunchHandoffReconciliationHistoryError::InvalidArg(format!(
                        "unknown argument for deploy record-launch-handoff-reconciliation-history: {arg}\n{}",
                        usage()
                    )),
                );
            }
...
fn parse_value<T, I>(
    args: &mut I,
    flag: &str,
) -> Result<T, DeploymentStackLaunchHandoffReconciliationHistoryError>
where
    T: std::str::FromStr,
    I: Iterator<Item = String>,
{
...
fn usage() -> &'static str {
    "usage: afterburner deploy record-launch-handoff-reconciliation-history --reconciliation PATH --event NAME --recorded-at-unix-ms MS [--out PATH]"
}
