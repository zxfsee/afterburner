...
- Deployment verification evidence bundle handoff adapter [Serving/Deployment Infra, Experimentation/Eval Infra]
  - Goal: Materialize the deployment verification evidence bundle handoff artifact over the current verification bundle so downstream deployment-side consumers can resolve one stable bundle entrypoint without surrounding rollout workspace assumptions.
  - Kind: `mixed`
