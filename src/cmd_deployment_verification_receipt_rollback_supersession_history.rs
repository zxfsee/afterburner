...
const DEFAULT_OUT_PATH: &str =
    "artifacts/deploy/deployment_verification_receipt_rollback_supersession_history.json";

#[derive(Debug)]
enum DeploymentVerificationReceiptRollbackSupersessionHistoryError {
    InvalidArg(String),
    Io(std::io::Error),
    Parse(String),
}

impl std::fmt::Display for DeploymentVerificationReceiptRollbackSupersessionHistoryError {
...
impl std::error::Error for DeploymentVerificationReceiptRollbackSupersessionHistoryError {}

impl From<afterburner::command_artifacts::JsonArtifactError>
    for DeploymentVerificationReceiptRollbackSupersessionHistoryError
{
    fn from(value: afterburner::command_artifacts::JsonArtifactError) -> Self {
        match value {
            ...
            afterburner::command_artifacts::JsonArtifactError::Serialize(err) => {
                Self::Parse(format!(
                    "serialize deployment verification receipt rollback supersession history json: {err}"
                ))
            }
        }
    }
...
impl From<ReconciliationError> for DeploymentVerificationReceiptRollbackSupersessionHistoryError {
...
fn run_inner<I>(
    args: I,
) -> Result<(), DeploymentVerificationReceiptRollbackSupersessionHistoryError>
where
    I: Iterator<Item = String>,
{
    ...
    write_emitted_json(
        "deploy_cli",
        "deployment_verification_receipt_rollback_supersession_history_written",
        "history_path",
        &args.out_path,
        "history",
        history,
...
fn parse_args<I>(
    args: I,
) -> Result<Args, DeploymentVerificationReceiptRollbackSupersessionHistoryError>
where
    I: Iterator<Item = String>,
{
        ...
        match arg.as_str() {
            ...
            _ if arg.starts_with("--recorded-at-unix-ms=") => recorded_at_unix_ms = Some(
                arg.trim_start_matches("--recorded-at-unix-ms=")
                    .parse::<u64>()
                    .map_err(|_| {
                        DeploymentVerificationReceiptRollbackSupersessionHistoryError::InvalidArg(
            ...
            _ => {
                return Err(
                    DeploymentVerificationReceiptRollbackSupersessionHistoryError::InvalidArg(
                        format!(
                            "unknown argument for deploy record-verification-receipt-rollback-supersession-history: {arg}\n{}",
                            usage()
                        ),
                    ),
                );
    ...
    Ok(Args {
        supersession: supersession.ok_or_else(|| {
            DeploymentVerificationReceiptRollbackSupersessionHistoryError::InvalidArg(format!(
...
fn parse_value<T, I>(
    args: &mut I,
    flag: &str,
) -> Result<T, DeploymentVerificationReceiptRollbackSupersessionHistoryError>
where
    T: std::str::FromStr,
    I: Iterator<Item = String>,
{
