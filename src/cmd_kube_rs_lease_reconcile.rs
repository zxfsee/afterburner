...
fn load_json_object(
    path: &Path,
    kind: &str,
) -> Result<serde_json::Map<String, Value>, KubeRsLeaseReconcileError> {
    let text = fs::read_to_string(path)?;
    let value: Value = serde_json::from_str(&text).map_err(|err| {
        KubeRsLeaseReconcileError::Parse(format!("parse {kind} `{}`: {err}", path.display()))
    })?;
    value.as_object().cloned().ok_or_else(|| {
        KubeRsLeaseReconcileError::Parse(format!("{kind} `{}` must be an object", path.display()))
    })
}
...
fn read_string(
    ...
    key: &'static str,
    path: &Path,
) -> Result<String, KubeRsLeaseReconcileError> {
    object
        .get(key)
        .and_then(Value::as_str)
        .map(str::to_string)
        .ok_or_else(|| {
            KubeRsLeaseReconcileError::Parse(format!(
                "gpu scheduler lease `{}` missing string field `{key}`",
                path.display()
            ))
        })
}
...
fn read_string_array(
    ...
    key: &'static str,
    path: &Path,
) -> Result<Vec<String>, KubeRsLeaseReconcileError> {
    object
        .get(key)
        .and_then(Value::as_array)
        .ok_or_else(|| {
            KubeRsLeaseReconcileError::Parse(format!(
                "gpu scheduler lease `{}` missing array field `{key}`",
                path.display()
            ))
        })?
        .iter()
        .map(|value| {
            value.as_str().map(str::to_string).ok_or_else(|| {
                KubeRsLeaseReconcileError::Parse(format!(
                    "gpu scheduler lease `{}` field `{key}` must contain only strings",
                    path.display()
                ))
            })
        })
