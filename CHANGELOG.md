...
- Inference drift baseline approval contract [Inference, Experimentation/Eval Infra]
  - Goal: Define an approval record for promoted infer drift baselines so the repo can distinguish a freshly computed baseline from one that has actually been approved for rollout decisions.
  - Kind: `mixed`
