...
- Inference drift baseline checkpoint contract [Inference, Experimentation/Eval Infra]
  - Goal: Define a compact checkpoint artifact bundling the current approved baseline pointer and recent drift-management records so rollout tooling can capture one inspectable recovery point before changing baseline state.
  - Kind: `mixed`
