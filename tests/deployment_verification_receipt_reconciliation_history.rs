...
fn deployment_verification_receipt_reconciliation_history_schema_and_workflow_are_explicit() {
    ...
    assert!(
        ...
        "justfile must expose the deployment-verification-record-receipt-reconciliation-history workflow"
    );

    let workflows = repo_file("docs/workflows.md");
    assert!(
        workflows.contains("deployment_verification_receipt_reconciliation_history.json"),
        "workflow reference must mention the deployment verification receipt reconciliation history artifact"
    );
    assert!(
        workflows.contains("just deployment-verification-record-receipt-reconciliation-history"),
        "workflow reference must mention the deployment verification receipt reconciliation history workflow"
    );

    let reference = repo_file("docs/reference.md");
    assert!(
        reference.contains("deployment_verification_receipt_reconciliation_history.json"),
        "reference index must mention the deployment verification receipt reconciliation history artifact"
    );
}
...
fn deployment_verification_receipt_reconciliation_history_writes_history_and_event() {
  ...
  "reconciliation_status": "aligned"
    ...
    cmd.arg("deploy")
        .arg("record-verification-receipt-reconciliation-history")
        .arg("--reconciliation")
        .arg(&reconciliation)
        .arg("--event")
        .arg("reconciled")
