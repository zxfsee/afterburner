...
- `just queue-top-runnable-check` fails when the top active TODO is blocked and reports the next runnable active item if one exists, otherwise the highest-priority runnable backlog item.
- `just queue-resume` is the explicit stale-lineage repair-and-continue path: it delegates to the typed queue preflight helper with stale-snapshot repair enabled so the repaired `CHANGELOG.md` baseline is carried forward unchanged before execution resumes.
...
- `just queue-promote-next-runnable` is the queue-only repair path for a blocked top active TODO: promote the next runnable active item when one exists, otherwise pull the highest-priority runnable backlog item into the active queue, then regenerate `CHANGELOG.md`, rerun the runnable-top check, and restamp the queue snapshot.
...
- `just queue-fix-top-scope` is the explicit stale-top-scope repair path: pin the narrow metadata objective, preserve unchanged in-scope worktree paths from the current top TODO, regenerate `CHANGELOG.md`, verify the stamped queue snapshot, then clear the transient lock.
- `just queue-execute-preflight` is the canonical execute-start path: it delegates queue freshness checks, optional stale-snapshot repair, execute pinning, and worktree validation to the typed queue preflight helper, carrying unchanged `Cargo.toml` and `CHANGELOG.md` forward when a top-scope repair already updated them.
...
- `just queue-execute-preflight --repair-stale-snapshot` keeps the same surface but runs the typed stale-lineage repair path first, then continues the normal preflight while carrying the repaired queue metadata into execute pinning unchanged.
