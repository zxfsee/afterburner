...
- Shared artifact/event emission helper extraction [Runtime Infra, Serving/Deployment Infra, Experimentation/Eval Infra]
  - Goal: Consolidate repeated artifact-writing and event-emission logic into one internal helper surface so adapter commands stop drifting in output paths, field handling, and write semantics as contracts grow.
  - Kind: `mixed`
  - Boundary: `none`
