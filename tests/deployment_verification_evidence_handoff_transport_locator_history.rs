...
#[test]
fn deployment_verification_handoff_transport_locator_history_schema_and_workflow_are_explicit() {
    let schema_text = fs::read_to_string(fixture_path(
        "deployment_verification_evidence_handoff_transport_locator_history.schema.json",
    ))
    .expect("read deployment verification handoff transport locator history schema");
    let schema: Value = serde_json::from_str(&schema_text)
        .expect("parse deployment verification handoff transport locator history schema");
...
#[test]
fn deployment_verification_handoff_transport_locator_history_writes_history_and_event() {
    ...
    fs::write(
        ...
        r#"{
  "schema_version": "1",
  "handoff_path": "artifacts/deploy/deployment_verification_evidence_handoff.json",
  ...
  "verification_status": "passed"
    ...
    let out = tmp
        .path()
        .join("deployment_verification_evidence_handoff_transport_locator_history.json");

    let mut cmd = cargo_bin_cmd!("afterburner");
    cmd.arg("debug")
        .arg("deploy")
        .arg("record-verification-handoff-transport-locator-history")
        .arg("--locator")
        .arg(&locator)
        .arg("--event")
        .arg("pointed")
    ...
    let event = stderr
        ...
        .find(|event| {
            event.get("event").and_then(Value::as_str)
                == Some("deployment_verification_evidence_handoff_transport_locator_history_written")
        })
        .unwrap_or_else(|| {
            panic!(
                "missing deployment verification handoff transport locator history event in stderr: {stderr}"
            )
        });
    ...
    assert_eq!(
        ...
        serde_json::json!({
            ...
            "level": "info",
            "source": "deploy_cli",
            "event": "deployment_verification_evidence_handoff_transport_locator_history_written",
            "fields": {
