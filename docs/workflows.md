...
- `just queue-top-runnable-check` fails when the top active TODO is blocked and reports the next runnable active item if one exists.
...
- `just queue-promote-next-runnable` is the queue-only repair path for a blocked top active TODO: promote the next runnable active item, regenerate `CHANGELOG.md`, rerun the runnable-top check, and restamp the queue snapshot.
...
- If `just queue-top-runnable-check` reports a blocked top active TODO, run `just queue-promote-next-runnable` before execution so the next runnable item becomes the active top slot.
