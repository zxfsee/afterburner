        ...
        match arg.as_str() {
            ...
            _ => {
                return Err(SchedulerHeartbeatPointerSupersessionError::InvalidArg(
                    format!(
                        "unknown argument for deploy scheduler-heartbeat-point-supersede: {arg}\n{}",
                        usage()
                    ),
                ));
            }
...
fn load_json_object(
    path: &Path,
    kind: &str,
) -> Result<serde_json::Map<String, Value>, SchedulerHeartbeatPointerSupersessionError> {
...
fn read_string(
    ...
    key: &'static str,
    path: &Path,
) -> Result<String, SchedulerHeartbeatPointerSupersessionError> {
    object
        .get(key)
...
fn read_u64(
    ...
    key: &'static str,
    path: &Path,
) -> Result<u64, SchedulerHeartbeatPointerSupersessionError> {
...
fn usage() -> &'static str {
    "usage: afterburner debug deploy scheduler-heartbeat-point-supersede --previous-pointer PATH --next-pointer PATH --superseded-at-unix-ms N [--out PATH]"
}
