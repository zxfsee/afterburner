...
#[test]
fn deployment_verification_handoff_transport_locator_schema_and_workflow_are_explicit() {
    let schema_text = fs::read_to_string(fixture_path(
        "deployment_verification_evidence_handoff_transport_locator.schema.json",
    ))
    .expect("read deployment verification handoff transport locator schema");
    let schema: Value = serde_json::from_str(&schema_text)
        .expect("parse deployment verification handoff transport locator schema");

    assert_eq!(
        schema.get("$id").and_then(Value::as_str),
        Some(
            "https://afterburner.local/schemas/deployment-verification-evidence-handoff-transport-locator/v1"
        )
    );
    let required = schema
        .get("required")
...
#[test]
fn deployment_verification_handoff_transport_locator_writes_locator_and_event() {
  ...
  "evidence_count": 2
    ...
    cmd.arg("debug")
        .arg("deploy")
        .arg("point-verification-handoff-transport-locator")
        .arg("--handoff")
        .arg(&handoff)
        .arg("--out")
        .arg(&out);
    ...
    .expect("read deployment verification handoff transport locator example");
    let mut expected: Value = serde_json::from_str(&expected_text)
        .expect("parse deployment verification handoff transport locator example");
    expected
        .as_object_mut()
    ...
    let event = stderr
        ...
        .find(|event| {
            event.get("event").and_then(Value::as_str)
                == Some("deployment_verification_evidence_handoff_transport_locator_written")
        })
        .unwrap_or_else(|| {
            panic!(
                "missing deployment verification handoff transport locator event in stderr: {stderr}"
            )
        });
    ...
    assert_eq!(
        ...
        serde_json::json!({
            ...
            "level": "info",
            "source": "deploy_cli",
            "event": "deployment_verification_evidence_handoff_transport_locator_written",
            "fields": {
