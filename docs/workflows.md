...
- `just queue-fix-top-scope` is the explicit stale-top-scope repair path: pin the narrow metadata objective, validate that only queue metadata is changing, regenerate `CHANGELOG.md`, verify the stamped queue snapshot, then clear the transient lock.
