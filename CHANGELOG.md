...
- Canonical `just` execution surface consolidation [Runtime Infra, Serving/Deployment Infra]
  - Goal: Converge user-facing workflows onto `just` as the canonical execution harness so cargo/manual command drift does not fragment reproducibility, documentation, or operator entrypoints.
  - Kind: `mixed`
