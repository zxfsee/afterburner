...
- Deployment stack launch receipt adapter [Serving/Deployment Infra, Runtime Infra]
  - Goal: Materialize the deployment stack launch receipt over the current launch plan and launch-time parameters so local and remote rollout tooling can audit one explicit launch record instead of shell history.
  - Kind: `mixed`
