...
- Do not avoid the canonical `just` formatter just because it is mutating in principle. If the tree is expected to be formatted already, `just fmt` should be a no-op and is still the correct dogfooding path; only drop to narrower formatter checks when the recipe is genuinely the wrong scope.
- Use `ast-grep` early when Rust work is structurally repetitive across sibling files or repeated layouts; use `rg` as the primary tool for literal strings, docs, `justfile`, exact symbol names, and quick scope discovery.
...
- Do not append a new low-priority queue item just to keep the visible horizon at six. Only add a follow-on when it is independently justified by the current work or required by actual queue-state repair, not by cosmetic horizon maintenance.
...
- Before executing a top `Kind: gate` TODO on a shared validator or shared docs-check surface, verify that the intended assertion still fails on the current tree. If it is already satisfied, do queue-only stale-top repair instead of treating it as runnable work; avoid splitting adjacent same-surface gate follow-ons unless each owns a distinct failing invariant.
...
- Do not bundle multiple small objective slices into one umbrella commit just because each diff is small. Keep each commit tied to one queued or explicitly chosen direct objective, and keep `NOTE.md` updates out of unrelated helper/gate rollouts unless the note itself is the primary objective being handled.
- When workflow drift appears in both `justfile` semantics and operator-facing CLI taxonomy, do not frame the fix as extraction alone. Pick one operator-facing family and land one end-to-end slice that moves recipe logic behind typed helpers while also collapsing the exposed command grammar to intent-first names.
- In grouped operator surfaces, changing `flat-artifact-taxonomy-command` into `family-manage taxonomy-heavy-action` is not enough. A regrouping is only finished when the remaining operator-visible segments are intent-first and most artifact/protocol taxonomy has moved into flags or internal dispatch, not just one level deeper in the command tree.
- Do not call `justfile` "thin orchestration" just because the worst hotspots improved. It is still not thin while queue-control branching, generic `action +args` buckets, or large artifact-family recipe expansions remain concentrated in the file.
- In this repo state, describe the `justfile` refactor as midpoint progress with concentrated remaining debt, not as completed thin orchestration. Treat the objective as unfinished until at least one remaining semantic choke point is removed end to end.
