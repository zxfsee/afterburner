...
- Inference drift baseline handoff manifest contract [Inference, Serving/Deployment Infra]
  - Goal: Define a minimal handoff manifest for exporting the current approved drift baseline state into deployment workflows so transport layers can consume one stable manifest instead of inferring which bundle fields are required.
  - Kind: `mixed`
