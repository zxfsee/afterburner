...
const DEFAULT_OUT_PATH: &str =
    "artifacts/deploy/gpu_scheduler_heartbeat_pointer_supersession_reconciliation.json";

#[derive(Debug)]
enum SchedulerHeartbeatPointerSupersessionReconcileError {
    InvalidArg(String),
    Io(std::io::Error),
    Parse(String),
}

impl std::fmt::Display for SchedulerHeartbeatPointerSupersessionReconcileError {
...
impl std::error::Error for SchedulerHeartbeatPointerSupersessionReconcileError {}

impl From<afterburner::command_artifacts::JsonArtifactError>
    for SchedulerHeartbeatPointerSupersessionReconcileError
{
    fn from(value: afterburner::command_artifacts::JsonArtifactError) -> Self {
        match value {
            ...
            afterburner::command_artifacts::JsonArtifactError::Serialize(err) => {
                Self::Parse(format!(
                    "serialize scheduler heartbeat pointer supersession reconciliation json: {err}"
                ))
            }
        }
    }
...
impl From<ReconciliationError> for SchedulerHeartbeatPointerSupersessionReconcileError {
...
fn run_inner<I>(args: I) -> Result<(), SchedulerHeartbeatPointerSupersessionReconcileError>
where
    I: Iterator<Item = String>,
{
    ...
    let reconciliation_status = if desired["previous_heartbeat_path"]
        == current["previous_heartbeat_path"]
        && desired["next_heartbeat_path"] == current["next_heartbeat_path"]
    ...
    write_emitted_json(
        "deploy_cli",
        "gpu_scheduler_heartbeat_pointer_supersession_reconciliation_written",
        "reconciliation_path",
        &args.out_path,
        "reconciliation",
        reconciliation,
...
fn supersession_payload(
    ...
    path: &std::path::Path,
) -> Result<Value, SchedulerHeartbeatPointerSupersessionReconcileError> {
    let mut payload = json!({
        "previous_heartbeat_path": read_string(supersession, "previous_heartbeat_path", path, "gpu scheduler heartbeat pointer supersession")?,
        "next_heartbeat_path": read_string(supersession, "next_heartbeat_path", path, "gpu scheduler heartbeat pointer supersession")?,
        "job_id": read_string(supersession, "job_id", path, "gpu scheduler heartbeat pointer supersession")?,
        "lease_id": read_string(supersession, "lease_id", path, "gpu scheduler heartbeat pointer supersession")?,
        "worker_id": read_string(supersession, "worker_id", path, "gpu scheduler heartbeat pointer supersession")?,
        "previous_state": read_string(supersession, "previous_state", path, "gpu scheduler heartbeat pointer supersession")?,
        "next_state": read_string(supersession, "next_state", path, "gpu scheduler heartbeat pointer supersession")?,
        "previous_observed_at_unix_ms": read_u64(supersession, "previous_observed_at_unix_ms", path, "gpu scheduler heartbeat pointer supersession")?,
        "next_observed_at_unix_ms": read_u64(supersession, "next_observed_at_unix_ms", path, "gpu scheduler heartbeat pointer supersession")?,
        "superseded_at_unix_ms": read_u64(supersession, "superseded_at_unix_ms", path, "gpu scheduler heartbeat pointer supersession")?,
    });
...
fn parse_args<I>(args: I) -> Result<Args, SchedulerHeartbeatPointerSupersessionReconcileError>
where
    I: Iterator<Item = String>,
{
...
fn parse_value<T, I>(
    args: &mut I,
    flag: &str,
) -> Result<T, SchedulerHeartbeatPointerSupersessionReconcileError>
where
    T: std::str::FromStr,
    I: Iterator<Item = String>,
{
