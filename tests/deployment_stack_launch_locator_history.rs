...
#[test]
fn deployment_stack_launch_locator_history_schema_and_workflow_are_explicit() {
    let schema_text = fs::read_to_string(fixture_path(
        "deployment_stack_launch_locator_history.schema.json",
    ))
    .expect("read deployment stack launch locator history schema");
    let schema: Value = serde_json::from_str(&schema_text)
        .expect("parse deployment stack launch locator history schema");
...
#[test]
fn deployment_stack_launch_locator_history_writes_history_and_event() {
    ...
    fs::write(
        &pointer,
        r#"{"schema_version":"1","locator_path":"artifacts/deploy/deployment_stack_launch_transport_locator.json","profile_name":"staging-a","deploy_hostname":"staging.example.net","rollout_entrypoint":"/srv/afterburner/artifacts/inference/current"}"#,
    )
    ...
    let event = stderr
        ...
        .find(|event| {
            event.get("event").and_then(Value::as_str)
                == Some("deployment_stack_launch_locator_history_written")
        })
        .unwrap_or_else(|| {
            panic!("missing deployment stack launch locator history event in stderr: {stderr}")
        });
    ...
    assert_eq!(
        ...
        serde_json::json!({
            ...
            "level": "info",
            "source": "deploy_cli",
            "event": "deployment_stack_launch_locator_history_written",
            "fields": {
