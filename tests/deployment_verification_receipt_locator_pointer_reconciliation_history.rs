...
#[test]
fn deployment_verification_receipt_locator_pointer_reconciliation_history_schema_and_workflow_are_explicit()
 {
    let schema_text = fs::read_to_string(fixture_path(
        "deployment_verification_receipt_locator_pointer_reconciliation_history.schema.json",
    ))
    .expect("read deployment verification receipt locator reconciliation history schema");
    let schema: Value = serde_json::from_str(&schema_text)
        .expect("parse deployment verification receipt locator reconciliation history schema");
    ...
    assert!(
        ...
        "justfile must expose the deployment-verification-record-receipt-locator-reconciliation-history workflow"
    );
    ...
    assert!(
        ...
        "workflow reference must mention the deployment verification receipt locator pointer reconciliation history artifact"
    );
    assert!(
        workflows
            .contains("just deployment-verification-record-receipt-locator-reconciliation-history"),
        "workflow reference must mention the deployment verification receipt locator pointer reconciliation history workflow"
    );
}

#[test]
fn deployment_verification_receipt_locator_pointer_reconciliation_history_writes_history_and_event()
{
  ...
  "pointer_path": "artifacts/deploy/deployment_verification_receipt_locator_pointer.json",
  "desired": {
  ...
  "reconciliation_status": "aligned"
    ...
    let event = stderr
        ...
        .unwrap_or_else(|| {
            panic!(
                "missing deployment verification receipt locator pointer reconciliation history event in stderr: {stderr}"
            )
        });
    ...
    assert_eq!(
        ...
        serde_json::json!({
            ...
            "level": "info",
            "source": "deploy_cli",
            "event": "deployment_verification_receipt_locator_pointer_reconciliation_history_written",
            "fields": {
