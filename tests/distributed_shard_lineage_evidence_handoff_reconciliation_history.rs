...
#[test]
fn distributed_shard_lineage_handoff_reconciliation_history_schema_and_workflow_are_explicit() {
    let schema_text = fs::read_to_string(fixture_path(
        "distributed_shard_lineage_evidence_handoff_reconciliation_history.schema.json",
    ))
    .expect("read lineage handoff reconciliation history schema");
    let schema: Value = serde_json::from_str(&schema_text)
        .expect("parse lineage handoff reconciliation history schema");
...
#[test]
fn distributed_shard_lineage_handoff_reconciliation_history_writes_history_and_event() {
  ...
  "reconciliation_status": "aligned"
    ...
    let out = tmp
        .path()
        .join("distributed_shard_lineage_evidence_handoff_reconciliation_history.json");
    ...
    let event = stderr
        ...
        .find(|event| {
            event.get("event").and_then(Value::as_str)
                == Some("distributed_shard_lineage_evidence_handoff_reconciliation_history_written")
        })
        .unwrap_or_else(|| {
            panic!("missing lineage handoff reconciliation history event in stderr: {stderr}")
        });
    ...
    assert_eq!(
        ...
        serde_json::json!({
            ...
            "level": "info",
            "source": "train_cli",
            "event": "distributed_shard_lineage_evidence_handoff_reconciliation_history_written",
            "fields": {
