...
- Pretraining dataset manifest contract [Data Infra, Pre-training]
  - Goal: Define a dataset-level manifest that pins corpus revision, shard inventory, split counts, and checksum rollups so large-scale pretraining data refreshes stay auditable without inferring from per-sample metadata alone.
  - Kind: `mixed`
