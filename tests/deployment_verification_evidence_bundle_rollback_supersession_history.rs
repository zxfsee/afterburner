...
#[test]
fn deployment_verification_bundle_rollback_supersession_history_schema_and_workflow_are_explicit() {
    let schema_text = fs::read_to_string(fixture_path(
        "deployment_verification_evidence_bundle_rollback_supersession_history.schema.json",
    ))
    .expect("read deployment verification bundle rollback supersession history schema");
    let schema: Value = serde_json::from_str(&schema_text)
        .expect("parse deployment verification bundle rollback supersession history schema");
    ...
    assert!(
        ...
        "justfile must expose the deployment-verification-record-bundle-rollback-supersession-history workflow"
    );

    let workflows = repo_file("docs/workflows.md");
    assert!(
        workflows
            .contains("deployment_verification_evidence_bundle_rollback_supersession_history.json"),
        "workflow reference must mention the deployment verification bundle rollback supersession history artifact"
    );
    assert!(
        workflows
            .contains("just deployment-verification-record-bundle-rollback-supersession-history"),
        "workflow reference must mention the deployment verification bundle rollback supersession history workflow"
    );
}

#[test]
fn deployment_verification_bundle_rollback_supersession_history_writes_history_and_event() {
    let tmp = tempfile::tempdir().expect("tempdir");
    let supersession = tmp
        .path()
        .join("deployment_verification_evidence_bundle_rollback_supersession.json");
    fs::write(
        &supersession,
        r#"{
  "schema_version": "1",
  ...
  "superseded_at_unix_ms": 1735689900000
    ...
    let out = tmp
        .path()
        .join("deployment_verification_evidence_bundle_rollback_supersession_history.json");

    let mut cmd = cargo_bin_cmd!("afterburner");
    cmd.arg("deploy")
        .arg("record-verification-bundle-rollback-supersession-history")
        .arg("--supersession")
        .arg(&supersession)
        .arg("--event")
        .arg("superseded")
    ...
    let event = stderr
        ...
        .unwrap_or_else(|| {
            panic!(
                "missing deployment verification bundle rollback supersession history event in stderr: {stderr}"
            )
        });
    ...
    assert_eq!(
        ...
        serde_json::json!({
            ...
            "level": "info",
            "source": "deploy_cli",
            "event": "deployment_verification_evidence_bundle_rollback_supersession_history_written",
            "fields": {
