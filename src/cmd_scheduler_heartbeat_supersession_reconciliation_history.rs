...
const DEFAULT_OUT_PATH: &str =
    "artifacts/deploy/gpu_scheduler_heartbeat_supersession_reconciliation_history.json";

#[derive(Debug)]
enum SchedulerHeartbeatSupersessionReconciliationHistoryError {
    InvalidArg(String),
    Io(std::io::Error),
    Parse(String),
}

impl std::fmt::Display for SchedulerHeartbeatSupersessionReconciliationHistoryError {
...
impl std::error::Error for SchedulerHeartbeatSupersessionReconciliationHistoryError {}

impl From<afterburner::command_artifacts::JsonArtifactError>
    for SchedulerHeartbeatSupersessionReconciliationHistoryError
{
    fn from(value: afterburner::command_artifacts::JsonArtifactError) -> Self {
        match value {
            ...
            afterburner::command_artifacts::JsonArtifactError::Serialize(err) => {
                Self::Parse(format!(
                    "serialize scheduler heartbeat supersession reconciliation history json: {err}"
                ))
            }
        }
    }
...
impl From<ReconciliationError> for SchedulerHeartbeatSupersessionReconciliationHistoryError {
...
fn run_inner<I>(args: I) -> Result<(), SchedulerHeartbeatSupersessionReconciliationHistoryError>
where
    I: Iterator<Item = String>,
{
    ...
    let reconciliation = load_json_object(
        ...
        "gpu scheduler heartbeat supersession reconciliation",
    )?;
    ...
    let entry = json!({
        ...
        "supersession_path": read_string(&reconciliation, "supersession_path", args.reconciliation.as_path(), "gpu scheduler heartbeat supersession reconciliation")?,
        "current_supersession_path": read_string(&reconciliation, "current_supersession_path", args.reconciliation.as_path(), "gpu scheduler heartbeat supersession reconciliation")?,
        "reconciliation_status": read_string(&reconciliation, "reconciliation_status", args.reconciliation.as_path(), "gpu scheduler heartbeat supersession reconciliation")?,
        "desired": read_object(&reconciliation, "desired", args.reconciliation.as_path(), "gpu scheduler heartbeat supersession reconciliation")?,
        "current": read_object(&reconciliation, "current", args.reconciliation.as_path(), "gpu scheduler heartbeat supersession reconciliation")?,
    ...
    write_emitted_json(
        "deploy_cli",
        "gpu_scheduler_heartbeat_supersession_reconciliation_history_written",
        "history_path",
        &args.out_path,
        "history",
        history,
...
fn parse_args<I>(args: I) -> Result<Args, SchedulerHeartbeatSupersessionReconciliationHistoryError>
where
    I: Iterator<Item = String>,
{
            ...
            _ if arg.starts_with("--recorded-at-unix-ms=") => {
                recorded_at_unix_ms = Some(
                    arg.trim_start_matches("--recorded-at-unix-ms=")
                        .parse::<u64>()
                        .map_err(|_| {
                            SchedulerHeartbeatSupersessionReconciliationHistoryError::InvalidArg(
...
fn parse_value<T, I>(
    args: &mut I,
    flag: &str,
) -> Result<T, SchedulerHeartbeatSupersessionReconciliationHistoryError>
where
    T: std::str::FromStr,
    I: Iterator<Item = String>,
{
...
fn usage() -> &'static str {
    "usage: afterburner deploy record-scheduler-heartbeat-supersession-reconciliation-history --reconciliation PATH --event NAME --recorded-at-unix-ms MS [--out PATH]"
}
