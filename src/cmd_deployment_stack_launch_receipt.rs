...
fn load_json_object(
    path: &Path,
    kind: &str,
) -> Result<serde_json::Map<String, Value>, DeploymentStackLaunchReceiptError> {
...
fn read_string(
    ...
    key: &'static str,
    path: &Path,
) -> Result<String, DeploymentStackLaunchReceiptError> {
    object
        .get(key)
        .and_then(Value::as_str)
        .map(str::to_string)
        .ok_or_else(|| {
            DeploymentStackLaunchReceiptError::Parse(format!(
                "deployment stack launch plan `{}` missing string field `{key}`",
                path.display()
            ))
        })
}
