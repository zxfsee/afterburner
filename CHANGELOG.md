...
- Workflow surface consistency checker gate [Runtime Infra]
  - Goal: Assert that new artifact-family workflows land with their CLI entrypoint, `just` recipe, schema fixture, example fixture, workflow reference, and reference-index mention together so contract surfaces stop drifting across the repo-managed harness.
  - Kind: `gate`
  - Boundary: `repo-workflow`
  - Contracts: `ops`
  - Scope: `src/bin/`, `tests/`, `fixtures/`, `docs/workflows.md`, `docs/reference.md`

- Reconciliation helper layer [Runtime Infra]
  - Goal: Extract a small shared helper for desired/current reconciliation artifacts so sibling bundle, handoff, and reconciliation-history adapters stop duplicating parse, compare, write, and event-emission plumbing while preserving explicit contract outputs.
  - Kind: `mixed`
  - Boundary: `adapter-cli`
  - Contracts: `artifact`, `event`, `ops`
  - Scope: `src/`, `tests/`

<!-- queue-snapshot: todo_sha256=febfe806b55aec429a2b7c813a08c4d8af149227bcaaf63a93b5e598153ae7a4 parent_commit=423a676ec7dfd47886c628432040f56827123f02 -->
