...
fn load_json_object(
    path: &Path,
    kind: &str,
) -> Result<serde_json::Map<String, Value>, DeploymentVerificationBundleRollbackReconcileError> {
    let text = fs::read_to_string(path)?;
    let value: Value = serde_json::from_str(&text).map_err(|err| {
        DeploymentVerificationBundleRollbackReconcileError::Parse(format!(
            "parse {kind} `{}`: {err}",
            path.display()
        ))
    })?;
...
fn read_string(
    ...
    path: &Path,
    kind: &str,
) -> Result<String, DeploymentVerificationBundleRollbackReconcileError> {
    object
        .get(key)
...
fn read_u64(
    ...
    path: &Path,
    kind: &str,
) -> Result<u64, DeploymentVerificationBundleRollbackReconcileError> {
