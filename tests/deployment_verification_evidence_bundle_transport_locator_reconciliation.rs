...
#[test]
fn deployment_verification_bundle_transport_locator_reconciliation_schema_and_workflow_are_explicit()
 {
    let schema_text = fs::read_to_string(fixture_path(
        "deployment_verification_evidence_bundle_transport_locator_reconciliation.schema.json",
    ))
    .expect("read deployment verification bundle transport locator reconciliation schema");
    let schema: Value = serde_json::from_str(&schema_text)
        .expect("parse deployment verification bundle transport locator reconciliation schema");

    assert_eq!(
        schema.get("$id").and_then(Value::as_str),
        Some(
            "https://afterburner.local/schemas/deployment-verification-evidence-bundle-transport-locator-reconciliation/v1"
        )
    );
    let required = schema
        .get("required")
    ...
    assert!(
        justfile
            .contains("deployment-verification-reconcile-bundle-transport-locator bundle locator:"),
        "justfile must expose the deployment-verification-reconcile-bundle-transport-locator workflow"
    );
    ...
    assert!(
        ...
        "workflow reference must mention the deployment verification bundle transport locator reconciliation artifact"
    );
    assert!(
        workflows.contains("just deployment-verification-reconcile-bundle-transport-locator"),
        "workflow reference must mention the deployment verification bundle transport locator reconciliation workflow"
    );
}

#[test]
fn deployment_verification_bundle_transport_locator_reconciliation_writes_artifact_and_event() {
  ...
  "verification_status": "passed"
    ...
    current.insert("bundle_path".to_string(), Value::from("<bundle>"));
    ...
    .expect("read deployment verification bundle transport locator reconciliation example");
    let mut expected: Value = serde_json::from_str(&expected_text)
        .expect("parse deployment verification bundle transport locator reconciliation example");
    expected
        .as_object_mut()
    ...
    expected_current.insert("bundle_path".to_string(), Value::from("<bundle>"));
    ...
    let event = stderr
        ...
        .find(|event| {
            event.get("event").and_then(Value::as_str)
                == Some("deployment_verification_evidence_bundle_transport_locator_reconciliation_written")
        })
        .unwrap_or_else(|| {
            panic!(
                "missing deployment verification bundle transport locator reconciliation event in stderr: {stderr}"
            )
        });
    ...
    event_current.insert("bundle_path".to_string(), Value::from("<bundle>"));
    assert_eq!(
        normalized_event,
        serde_json::json!({
            "ts_ms": 0,
            "level": "info",
            "source": "deploy_cli",
            "event": "deployment_verification_evidence_bundle_transport_locator_reconciliation_written",
            "fields": {
