...
#[test]
fn gpu_scheduler_heartbeat_pointer_rollback_reconciliation_schema_and_workflow_are_explicit() {
    ...
    assert!(
        justfile
            .contains("scheduler-heartbeat-point-rollback-reconcile rollback current_rollback:"),
        "justfile must expose the scheduler-heartbeat-point-rollback-reconcile workflow"
    );

    let workflows = repo_file("docs/workflows.md");
    assert!(
        workflows.contains("gpu_scheduler_heartbeat_pointer_rollback_reconciliation.json"),
        "workflow reference must mention the pointer rollback reconciliation artifact"
    );
    assert!(
        workflows.contains("just scheduler-heartbeat-point-rollback-reconcile"),
        "workflow reference must mention the pointer rollback reconciliation workflow"
    );

    let reference = repo_file("docs/reference.md");
    assert!(
        reference.contains("gpu_scheduler_heartbeat_pointer_rollback_reconciliation.json"),
        "reference index must mention the pointer rollback reconciliation artifact"
    );
}
