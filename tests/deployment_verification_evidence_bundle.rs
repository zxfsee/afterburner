...
fn deployment_verification_evidence_bundle_schema_and_workflow_are_explicit() {
    ...
    assert!(
        ...
        "README must mention the deployment verification evidence bundle artifact"
    );
    assert!(
        readme.contains("just deployment-verification-bundle"),
        "README must mention the deployment verification evidence bundle workflow"
    );
}
...
fn deployment_verification_bundle_writes_bundle_and_event() {
    ...
    cmd.arg("deployment-verification-bundle")
        .arg("--receipt")
        .arg(&receipt)
        .arg("--out")
        .arg(&out);
