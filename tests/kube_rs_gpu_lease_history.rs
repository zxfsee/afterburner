...
#[test]
fn kube_rs_gpu_lease_history_schema_and_workflow_are_explicit() {
    let schema_text = fs::read_to_string(fixture_path("kube_rs_gpu_lease_history.schema.json"))
        .expect("read kube-rs lease history schema");
    let schema: Value = serde_json::from_str(&schema_text).expect("parse kube-rs lease history");
    ...
    assert!(
        workflows.contains("kube_rs_gpu_lease_history.json"),
...
#[test]
fn kube_rs_gpu_lease_history_writes_history_and_event() {
    let tmp = tempfile::tempdir().expect("tempdir");
    let pointer = tmp.path().join("kube_rs_gpu_lease_pointer.json");
    fs::write(
        &pointer,
        r#"{"schema_version":"1","reconciliation_path":"artifacts/deploy/kube_rs_gpu_lease_reconciliation.json","lease_id":"lease-job-0001","job_id":"job-0001","node_id":"node-a","gpu_ids":["0"],"checkpoint_mode":"cooperative","namespace":"afterburner-system","resource_name":"lease-job-0001","desired_state":"present"}"#,
    )
            ...
            "entries": [
                {
                    ...
                    "reconciliation_path": "artifacts/deploy/kube_rs_gpu_lease_reconciliation.json",
    ...
    let event = stderr
        ...
        .find(|event| {
            event.get("event").and_then(Value::as_str) == Some("kube_rs_gpu_lease_history_written")
        })
        .unwrap_or_else(|| panic!("missing kube-rs lease history event in stderr: {stderr}"));
    ...
    assert_eq!(
        ...
        serde_json::json!({
            ...
            "level": "info",
            "source": "deploy_cli",
            "event": "kube_rs_gpu_lease_history_written",
            "fields": {
