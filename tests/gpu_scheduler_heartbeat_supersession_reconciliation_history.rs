...
fn gpu_scheduler_heartbeat_supersession_reconciliation_history_schema_and_workflow_are_explicit() {
    ...
    .expect("read heartbeat supersession reconciliation history schema");
    let schema: Value = serde_json::from_str(&schema_text)
        .expect("parse heartbeat supersession reconciliation history schema");
...
#[test]
fn gpu_scheduler_heartbeat_supersession_reconciliation_history_writes_history_and_event() {
    ...
    fs::write(
        ...
        r#"{
  "schema_version": "1",
  "supersession_path": "artifacts/deploy/gpu_scheduler_heartbeat_supersession.json",
  "current_supersession_path": "artifacts/deploy/gpu_scheduler_heartbeat_supersession.current.json",
  "desired": {
    "job_id": "job-0001",
  ...
  "reconciliation_status": "aligned"
    ...
    let out = tmp
        .path()
        .join("gpu_scheduler_heartbeat_supersession_reconciliation_history.json");
    ...
    let event = stderr
        ...
        .find(|event| {
            event.get("event").and_then(Value::as_str)
                == Some("gpu_scheduler_heartbeat_supersession_reconciliation_history_written")
        })
        .unwrap_or_else(|| {
            panic!(
                "missing gpu_scheduler_heartbeat_supersession_reconciliation_history_written event in stderr: {stderr}"
            )
        });
    ...
    assert_eq!(
        ...
        serde_json::json!({
            ...
            "level": "info",
            "source": "deploy_cli",
            "event": "gpu_scheduler_heartbeat_supersession_reconciliation_history_written",
            "fields": {
