...
const DEFAULT_OUT_PATH: &str =
    "artifacts/deploy/deployment_stack_launch_evidence_bundle_reconciliation_history.json";

#[derive(Debug)]
enum DeploymentStackLaunchBundleReconciliationHistoryError {
    InvalidArg(String),
    Io(std::io::Error),
    Parse(String),
}

impl std::fmt::Display for DeploymentStackLaunchBundleReconciliationHistoryError {
...
impl std::error::Error for DeploymentStackLaunchBundleReconciliationHistoryError {}

impl From<std::io::Error> for DeploymentStackLaunchBundleReconciliationHistoryError {
    fn from(value: std::io::Error) -> Self {
        Self::Io(value)
    }
}

impl From<JsonArtifactError> for DeploymentStackLaunchBundleReconciliationHistoryError {
    fn from(value: JsonArtifactError) -> Self {
        match value {
            JsonArtifactError::Io(err) => Self::Io(err),
            JsonArtifactError::Serialize(err) => Self::Parse(format!(
                "serialize deployment stack launch evidence bundle reconciliation history json: {err}"
            )),
        }
    }
}
...
fn run_inner<I>(args: I) -> Result<(), DeploymentStackLaunchBundleReconciliationHistoryError>
where
    I: Iterator<Item = String>,
{
    ...
    emit_json_artifact_written(
        "deploy_cli",
        "deployment_stack_launch_evidence_bundle_reconciliation_history_written",
        "history_path",
        &args.out_path,
        "history",
        history,
...
fn parse_args<I>(args: I) -> Result<Args, DeploymentStackLaunchBundleReconciliationHistoryError>
where
    I: Iterator<Item = String>,
{
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ...
            _ if arg.starts_with("--recorded-at-unix-ms=") => {
                recorded_at_unix_ms = Some(
                    arg.trim_start_matches("--recorded-at-unix-ms=")
                        .parse::<u64>()
                        .map_err(|_| {
                            DeploymentStackLaunchBundleReconciliationHistoryError::InvalidArg(
            ...
            _ => {
                return Err(
                    DeploymentStackLaunchBundleReconciliationHistoryError::InvalidArg(format!(
                        "unknown argument for deploy record-launch-bundle-reconciliation-history: {arg}\n{}",
                        usage()
                    )),
                );
            }
...
fn parse_value<T, I>(
    args: &mut I,
    flag: &str,
) -> Result<T, DeploymentStackLaunchBundleReconciliationHistoryError>
where
    T: std::str::FromStr,
    I: Iterator<Item = String>,
{
...
fn load_json_object(
    path: &Path,
    kind: &str,
) -> Result<serde_json::Map<String, Value>, DeploymentStackLaunchBundleReconciliationHistoryError> {
    let text = fs::read_to_string(path)?;
    let value: Value = serde_json::from_str(&text).map_err(|err| {
        DeploymentStackLaunchBundleReconciliationHistoryError::Parse(format!(
            "parse {kind} `{}`: {err}",
            path.display()
        ))
    })?;
...
fn load_history(
    path: &Path,
) -> Result<Value, DeploymentStackLaunchBundleReconciliationHistoryError> {
    let text = fs::read_to_string(path)?;
    let history: Value = serde_json::from_str(&text).map_err(|err| {
        DeploymentStackLaunchBundleReconciliationHistoryError::Parse(format!(
            "parse deployment stack launch evidence bundle reconciliation history `{}`: {err}",
            path.display()
        ))
    })?;
    history
        ...
        .ok_or_else(|| {
            DeploymentStackLaunchBundleReconciliationHistoryError::Parse(format!(
                "deployment stack launch evidence bundle reconciliation history `{}` must contain an `entries` array",
                path.display()
            ))
        })?;
    Ok(history)
...
fn read_string(
    ...
    key: &'static str,
    path: &Path,
) -> Result<String, DeploymentStackLaunchBundleReconciliationHistoryError> {
    object
        .get(key)
        ...
        .ok_or_else(|| {
            DeploymentStackLaunchBundleReconciliationHistoryError::Parse(format!(
                "deployment stack launch evidence bundle reconciliation `{}` missing string field `{key}`",
                path.display()
            ))
        })
}
...
fn read_object(
    ...
    object.get(key).cloned().ok_or_else(|| {
        DeploymentStackLaunchBundleReconciliationHistoryError::Parse(format!(
            "deployment stack launch evidence bundle reconciliation `{}` missing field `{key}`",
            path.display()
        ))
    })
}
