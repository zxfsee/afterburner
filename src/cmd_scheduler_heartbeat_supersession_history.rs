...
const DEFAULT_OUT_PATH: &str = "artifacts/deploy/gpu_scheduler_heartbeat_supersession_history.json";

#[derive(Debug)]
enum SchedulerHeartbeatSupersessionHistoryError {
    InvalidArg(String),
    Io(std::io::Error),
    Parse(String),
}

impl std::fmt::Display for SchedulerHeartbeatSupersessionHistoryError {
...
impl std::error::Error for SchedulerHeartbeatSupersessionHistoryError {}

impl From<afterburner::command_artifacts::JsonArtifactError>
    for SchedulerHeartbeatSupersessionHistoryError
{
    fn from(value: afterburner::command_artifacts::JsonArtifactError) -> Self {
        match value {
            ...
            afterburner::command_artifacts::JsonArtifactError::Serialize(err) => Self::Parse(
                format!("serialize scheduler heartbeat supersession history json: {err}"),
            ),
        }
    }
}

impl From<ReconciliationError> for SchedulerHeartbeatSupersessionHistoryError {
...
fn run_inner<I>(args: I) -> Result<(), SchedulerHeartbeatSupersessionHistoryError>
where
    I: Iterator<Item = String>,
{
    ...
    let mut entry = json!({
        ...
        "job_id": read_string(&supersession, "job_id", args.supersession.as_path(), "gpu scheduler heartbeat supersession")?,
        "lease_id": read_string(&supersession, "lease_id", args.supersession.as_path(), "gpu scheduler heartbeat supersession")?,
        "worker_id": read_string(&supersession, "worker_id", args.supersession.as_path(), "gpu scheduler heartbeat supersession")?,
        "previous_state": read_string(&supersession, "previous_state", args.supersession.as_path(), "gpu scheduler heartbeat supersession")?,
        "next_state": read_string(&supersession, "next_state", args.supersession.as_path(), "gpu scheduler heartbeat supersession")?,
        "previous_observed_at_unix_ms": read_u64(&supersession, "previous_observed_at_unix_ms", args.supersession.as_path(), "gpu scheduler heartbeat supersession")?,
        "next_observed_at_unix_ms": read_u64(&supersession, "next_observed_at_unix_ms", args.supersession.as_path(), "gpu scheduler heartbeat supersession")?,
        "superseded_at_unix_ms": read_u64(&supersession, "superseded_at_unix_ms", args.supersession.as_path(), "gpu scheduler heartbeat supersession")?,
    ...
    write_emitted_json(
        "deploy_cli",
        "gpu_scheduler_heartbeat_supersession_history_written",
        "history_path",
        &args.out_path,
        "history",
        history,
...
fn parse_args<I>(args: I) -> Result<Args, SchedulerHeartbeatSupersessionHistoryError>
where
    I: Iterator<Item = String>,
{
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ...
            _ if arg.starts_with("--recorded-at-unix-ms=") => {
                recorded_at_unix_ms = Some(
                    arg.trim_start_matches("--recorded-at-unix-ms=")
                        .parse::<u64>()
                        .map_err(|_| {
                            SchedulerHeartbeatSupersessionHistoryError::InvalidArg(format!(
            ...
            _ => {
                return Err(SchedulerHeartbeatSupersessionHistoryError::InvalidArg(
                    format!(
                        "unknown argument for deploy record-scheduler-heartbeat-supersession-history: {arg}\n{}",
                        usage()
                    ),
                ));
            }
...
fn parse_value<T, I>(
    args: &mut I,
    flag: &str,
) -> Result<T, SchedulerHeartbeatSupersessionHistoryError>
where
    T: std::str::FromStr,
    I: Iterator<Item = String>,
{
...
fn usage() -> &'static str {
    "usage: afterburner debug deploy record-scheduler-heartbeat-supersession-history --supersession PATH --event NAME --recorded-at-unix-ms MS [--out PATH]"
}
