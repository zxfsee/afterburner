...
- Distributed shard lineage handoff history adapter [Distributed Training, Data Infra]
  - Goal: Append one compact history artifact over distributed shard lineage evidence handoff changes so downstream lineage tooling can audit handoff updates without reading shell or process logs.
  - Kind: `mixed`
  - Boundary: `core-contract`
  - Contracts: `artifact`, `event`
  - Scope: `src/`, `fixtures/`, `tests/`, `README.md`, `docs/adr/`

- Deployment stack launch locator reconciliation history adapter [Serving/Deployment Infra, Runtime Infra]
  - Goal: Append one compact history artifact over deployment stack launch locator reconciliation changes so downstream deployment tooling can audit desired-versus-current locator updates without reading shell or process logs.
  - Kind: `mixed`
  - Boundary: `adapter-deployment`
  - Contracts: `artifact`, `event`, `ops`
